...
## 三点五、地面口径与城市布局生成（地面 2026-09-16 创始人定稿；布局 2026-09-15 定稿）

**地面两态（城内一个材质、城外全草地）**（沿 z 从远到近，横向随城市宽度延展；贴图均在 tex/ground_tiles/）：

| 带 | 范围 | 城内（墙线以内） | 城外（墙外） |
|---|---|---|---|
| **建筑带**（台面，z 0.42~1.95，高 PLAT_H=0.65） | 建筑基座 + 街具 | 石板台面（flagstone，**拉通到墙线**） | —— |
| **道路带**（z 1.9~46，行走面，y=0） | 玩家/村民行走 | 石板路（band_road_stone，**拉通到墙线**） | 草地（草底层露出） |
| **背景地面带**（城内，远端→0.42，**y=PLAT_H 与建筑带同高**） | 城内地面延续到地平线 | 石板（flagstone，与台面同材质，拉通到墙线） | 草地条（grass_alb **淡草色** 0.90/0.93/0.80，铺到地图边缘 ±(墙线+30)——远处必须读作"地"） |

- **草底层**：整幅草地（grass_alb，**淡草色 0.90/0.93/0.80**，与远景墙外段同色不断缝）从地平线贯通铺到近缘（y=0，不分段），城内石板分段压其上；兜底大地皮同步用草（防视野越出分段露底时窜出异质色）。（创始人 2026-09-16：城外全是草地，稀疏草土退役；城内全部中心城区石板，城乡过渡材质——边缘区草地分幅/夯土带/gtx 交接贴花/土坎镶边——整体退役，暂缓再作。）
- **平铺地皮不用条带图**：band_shoulder_stone 是贴墙根条带图，顶缘墙根暗带在世界锚定平铺下每 tile 复现成横贯的"污渍带"（创始人 2026-09-16 指认），平铺地皮一律用 flagstone 干净方板；条带图只留给真正贴墙根/立边的场合（如墙体三平面投射）。

**天空与天体**：天空 = **贴图天幕**（`_build_sky` 的相机子 quad，纯蓝临时口径
`SKY_BLUE`，换手绘贴图只动该处材质）；不用 ProceduralSkyMaterial——26° 俯角
正交相机下其天空/地面半球分界压在屏幕中线，蓝天画不进"地面远端→中线"带，
读作天空只盖上半屏。天幕深度 120 在地面远端/楼群之后，地平线分界由深度测试
天然压出；背景模式退 `BG_COLOR` 作天幕外兜底。

**日/月天体**：**太阳 = 屏幕后处理炫光**（创始人 2026-09-16：就是
PostProcessLayer 的屏幕太阳 glow+streak+ghost，要它沿弧线到处移动）——场景
里不摆任何太阳贴片，`_update_celestial` 每帧把弧线屏幕位经
`PostProcessLayer.set_sun_uv()` 写进 shader（独立渲染无 SystemSetup 时自建该
层）。弧线公式直译 2D SkyStars（Terraria DrawSunAndMoon）：5..21 时线性横穿
0.65 视宽、抛物线高度（正午最高 0.685 半视高、晨昏下沉 0.463）；炫光明暗由
后处理层昼夜联动负责（夜晚 0.22 残辉、暂停淡出）。月亮 19..29 时（跨午夜）
同弧线，**月亮 = 天幕上的实心小白盘**（2D SkyStars 同款语义），夜间炫光跟随
月位。太阳/月亮切换按天空底色亮度门控。时刻源 `WorldState.game_time`，缺失
按正午。**主光 `_sun` 方向仍按档位固定不随天体走**（渲染确定性）；昼档光强
1.10（创始人拍板的高调口径，覆盖旧"环境光+太阳≈1.0"配平基线）。

草地规则：城内全是石板铺装（不长草），城外全是草地（grass_alb 单档；稀疏草土已退役，2026-09-16）。
...
- 地面贴图（2026-09-15 定稿）：**全部世界坐标锚定 UV**（同材质跨带无缝续接、缩放全局一致）；背景地面带与建筑带**同高**——建筑带身后的城内地面保持台面标高到地平线，无"踩空"落差（创始人 2026-09-15），同材质（flagstone）拉通到墙线（2026-09-16），只铺到**第二排后景根部**（地平线=第二排楼脚）；墙外=草底层整幅贯通。
...
逆变换（屏幕→世界）：  x 按仿射逆；y = 1294 − (vp_h − screen_y)/(k·ez)
                     （map.screen_y_to_ground_y，F3 鼠标世界坐标用；台面区
                     未含抬升逆解，鼠标读数为路面口径近似）
```
...
3. HD-2D 图 origin=**视觉脚线**（billboard 脚锚），悬浮框 billboard 几何 =
   ...
   **白色选中框=全身包裹**（框=悬浮框同一矩形 `entity_hover_rect`，角臂画
   ...
   四者共用同一矩形；蓝 F3 碰撞框画在物理碰撞位（物理脚印已贴脚线，两口径
   合一，见 4.4/4.5）。
4. 调用链上已有 remap 的出口（如 `FxPool.spawn_burst` 内部 remap 地面锚），
   上游传视觉域坐标前须先 `unmap_fx_pos` 逆回，防二次压缩。
5. **缩放算进初始值**：矩形/尺寸统一在产出端过一次变换或由矩形自导出
   （如选中框角臂=屏幕矩形短边×0.15 钳制），**禁止画布像素直当屏幕像素**
   （曾致选中框比悬浮框大 1.33×）——zoom 补偿不在消费点各自附加。
   origin 空间图实体 Collider 居 origin（物理脚印=视觉脚线，物理/显示/视觉
   三者重合；2D 图维持 origin+foot_offset 旧口径，旗帜门控）。
...
- **地形分行线（terrain_grid）**：HD-2D 图**不画**——32px 分行语义属于 2D 图
  ...
  （创始人 2026-09-15）。
- **实体碰撞箱（entity_collider_drawer）**：HD-2D 图画在**物理碰撞位**（箱心
  经 remap_fx_pos 含台面抬升，宽高不压、x 用物理箱真实横向范围；F3=碰撞真相）。
  **物理脚印已贴视觉脚线**——origin 空间图实体 Collider 居 origin（视觉脚线，
  2026-09-16 定案，此前挂在 2D 髋部语义的 origin+130 上，物理脚印悬在视觉
  脚线"前方" ~130·k·ez，停位与视觉脱节）——物理位=脚下线框位，两口径合一，
  蓝紫相撞即停且视觉吻合。2D 图箱仍在 origin+foot_offset，维持原中心直绘。
- **障碍箱（蓝/紫 `_draw_area_rect`）**：y0/y1 各自 remap（地面占地投影）。
  HD2DSolids 里前排建筑的形状带 `hd2d_building` meta，此抽屉**跳过**——建筑的
  占地显示改由下条**紫占地带**承担、垂直范围由白包楼框表达，防双重绘制。
- **建筑紫占地带（building_drawer HD-2D 分支，2026-09-15 恢复）**：PassageBarrier
  紫口径（填充 0.3+描边 0.8）。范围口径（创始人 2026-09-15 两轮定案）：
  **宽度=占位槽宽（=白包楼框宽，"起码与白框同宽"）**；**深度=全模型占地深**
  （`footprint_full`[1]，含屋顶出檐——墙脚贴地实测对大屋顶建筑只是 1~3 格窄条
  "扁片"，故深度口径换全深），下限 2 格、后沿不越过深端行走界（黄线）。
...
- **鼠标世界坐标**：y 走 4.2 逆变换；标签钉鼠标屏幕位（世界 y 已是地面带坐标，
  不能再经 2D 投影回屏幕）。
- **选中/悬浮四角框（SelectionSystem）**：**全身包裹**——选中框与悬浮框共用
  `entity_hover_rect` 矩形（HD-2D=billboard 视觉身高 156 框，脚线~头顶；2D=
  Range 原框），角臂画四角（创始人 2026-09-16"框住整个火柴人"；§4.2.1 协议）。
  点选/框选判定锚同一矩形中心。possessed 主控框走 3D 脚下框
  （wants_3d_bracket 门控），位置天然正确。
- **建筑宽度辅助线**（2D 图 F3 版式样原样搬入，白色口径，创始人 2026-09-15）：
  ...
  - building_drawer → HD-2D 分支：每栋建筑**左右边界竖线**（跨该栋真实墙脚
    footprint 纵深带），白 0.6；数据口
    `proto.get_building_rects() → Hd2dStreetMap.get_building_rects()`，
    宽度口径=**真实墙脚宽**（footprint[0]，烘焙导出；画面宽含出檐、占位槽
    4 格整倍数，都不用于此线）。
  ...
  - ⚠ 占地带元组是**混合口径：x=格（×32 转 px）、y=px**——与 get_solid_rects
    ...
    后沿封顶在深端行走界；`footprint_off` 仍导出备用但当前带位不消费（宽度=槽位、
    前沿=墙脚基线已定案），
...
**已知口径（2026-09-16 统一，勿回退）**：origin 空间图（HD-2D 系）实体
**Collider 居 origin=视觉脚线**——物理脚印与视觉脚线重合，蓝紫相撞即停且
视觉吻合（此前 Collider 挂 2D 髋部语义的 origin+130，物理脚印悬在视觉脚线
"前方" ~130·k·ez，创始人指认"显示偏上、逻辑位置也偏很多"后统一）。2D 图
维持 origin=髋、Collider 在 origin+foot_offset 的旧口径（`_origin_space_
walk_band` 旗帜门控，实体 set_ground_constraints 注入）。视觉系（接地影/FX/
...
- 资源/村B 图继承 Hd2dStreetMap，契约自动生效；**战场图纵向取景走 `set_cam_zoom`
  战场分支**（地平线钉屏幕上 1/3 线、地面占下 2/3，见 4.1 末）；2D 旧图
  （village_map 系）走 MapBase 恒等协议（视觉域=画布域，见 4.2.1），行为不变。
...
| # | 层 | z | 要点 |
|---|---|---|---|
| 1 | 贴图天幕 | — | **相机子天幕 quad**（纯蓝 `SKY_BLUE` 临时口径，换手绘贴图只动 `_build_sky`）+ **屏幕后处理太阳（PostProcessLayer sun_uv）沿弧线驱动**/月亮盘（5..21 时太阳/19..29 时月亮，公式直译 2D SkyStars，见 §日/月天体）+ **漂移云牌** = 2D 手绘云（SketchCloud）烘贴图的 Sprite3D billboard（14 朵池、风驱漂移+出带回绕、昼白/夜暗蓝，z 钉背景卡面之后）；昼/夜色值都在 `_apply_light` |
| 2 | 远景地面带（城内） | far_z→0.42 | **与建筑带（台面）同高 y=PLAT_H**——建筑带身后的城内地面保持台面标高一直到地平线，无"踩空"落差（创始人 2026-09-15）；城内段石板（flagstone）**拉通到墙线**（2026-09-16：城内一个材质），墙外段淡草色草地条到地图边缘（2026-09-17：远处读作"地"，不用深绿 tint）；资源图维持旧分幅 y=0 平铺；**战场 = 全幅绿草单一材质 z 0→115，无分幅无铺装带**（创始人：整个大地一个材质） |
| 3 | 草底层（城外野地） | far_z→46 | 整幅草地（grass_alb 淡草色）**全宽贯通**（地平线→近缘不分段，y=0）——墙外露出即野地，城内段被石板分段覆盖（2026-09-16：城外全是草地，稀疏草土退役） |
| 4 | 兜底大地皮 | far_z→40 | 1200 格宽草皮（同草底层材质），y=−0.05 压在所有分段之下，防任意缩放露底；**远端收在第二排后景基线**（=真实地平线，"地平线=第二排楼脚"，§4.0 新地平线）；战场远端与主地皮对齐（z=0 即地平线——多铺会越过地平线顶掉 2/3 构图） |
...
| 9 | 街具道具 | 4.2~7.2（台面带只留杂物占格件+贴楼功能件，街具铺路面带 5.4~7.2） | 卡底留白 alpha 扫描下沉（防浮空）；点障碍碰撞（卡宽×0.85，灯笼不挡；纵深带=基线向街心延伸 52px、不前伸——前伸会让角色在基线前 ~38px 被挡，观感"碰撞箱下偏 ~32px"，创始人 2026-09-16）；铁砧=露天铁匠工位（§2.7）；高件避让门脸/杂物（CityGen §6.5） |
| 10 | 道路带 | 1.9~46 | 行走面 y=0：石板路（band_road_stone）**拉通到墙线**；墙外=草底层露出；角色 z 同 6.1 映射 |
...
- 消费映射：row0=前排（z 契约 0.45~1.3 台面 / 2.4 落地面直接采用）；props 直用；trees 现恒空——野外资源由宿主 resource_gen 算法（群落散布+林区梯度）撒点后 `spawn_nature_card_at` 落卡；**row≥1=背景第一层整排直接消费**（三层背景口径见 §2.2/§6.7）。
- **props z/plat 契约（2026-09-16 创始人定案：小件全撤、常识重摆）**：**每件道具必须有功能理由**——铁砧/磨刀石=铁匠工位（贴楼）、井=市集聚点、摊/桌/果篮=市集面（两翼展开：摊+果篮右翼、桌左翼）、旗帜=行政楼前（贴楼）、长椅=井旁对置（village 起，聚点语义）、喷泉=镇起广场位（市场锚点侧翼 +9/−9 择空；小村落没喷泉）、路标/里程碑=街口侧翼、灯=沿街照明（周期 8~12 格两侧/两档纵深错位，石/铁柱逐盏轮换）、杂物堆=仓储/生产带占格件（§3.5；盆/筐/花箱等又矮又小的小件全撤不上街）。**随机街具组轮转（长椅/花坛/水龙头等乱点配方）整体删除**。路肩台面带=建筑脚下 z 0.42~1.95，`plat=true` 只在台面带内成立（渲染端 z≥2 强制回路面守卫）。**带语义**：台面带只留"按建筑算"的东西（建筑+杂物占格件+贴楼功能小件），街具铺路面带（远缘 5.8 / 近缘 7.0）。**建筑间杂物与建筑同 Z 带占水平格**（z 按卡自身 footprint 纵深推导、0.42~1.75 内收；CityGen §3.5 塞入分区侧序列随机位置段，随建筑整格排布推挤；占格宽按真实地面占地 footprint[0] 向上取整，卡库缺失兜底 2 格；序列里小概率（9% 一格 / 3% 两格）塞占格空位——占格不出卡的刻意留白）。**同带互斥登记表**：道具/杂物落位即登记 [x0,x1,z]，后放者避让 z 差<1.5 格（PROP_EXCL_BAND）的已放件——道具↔道具零穿模；z 差≥1.5=前后遮挡合法（保留街景层次）；高件另避让门脸建筑段（避让候选先对齐 0.1 格再测，与落盘 snap 同口径）。**杂物进 F3 建筑辅助线**：CityGen 杂物带 `occ_cells` 占格宽字段，proto_hd2d 收进 `get_building_rects()`——双黄线+紫占地带与建筑同口径，碰撞仍走 `_prop_solids` 点障碍。**前排 2 层卡配额**（CityGen §3.7）：townlet 2 / town 5 / burgh 8 / city 9 / capital 14 / metropolis 18，白名单九卡每 def ≤2 张复制补位，档位可得性由分区池守级别窗口。
- **街具节奏（CityGen §6，port 自 props.py dress_street）**：灯距 8~12 格两侧错位、石/铁灯柱逐盏轮换（灯卡查 props.json 卡宽表，查建筑卡表会永远落灯笼兜底——"小灯当路灯"根因）；家具组每侧 furniture_n 槽均分街宽（组距 11~17 格），组内件按卡宽肩并肩、配方洗牌袋不连号复读；路标/里程碑只守街口不进组。生成器确定性依赖全部随机走 rng（`_shuffle_rng`）——`Array.shuffle()` 吃全局随机源，同种子会每进城重排。
...
- **day = 阳光明媚高调照明**：纯蓝天幕 + 主光暖白 **1.10**（创始人 2026-09-16 恢复提亮档原值；屏幕炫光太阳沿弧线驱动，主光方向固定不随天体走）+ 冷天空环境 0.58 + 冷补光 0.12 抬暗部（卡是烘焙图，场景光只做保底/氛围）；雾关；tonemap LINEAR（filmic/aces 会把烘焙卡压灰）。
- **night（月夜档）**：天幕近黑蓝 + 环境 0.42 + 月亮方向光 0.15（月光主要烘在卡里，能量一高画面整体推蓝）+ 雾关 + 窗火 glow 0.7 + 街灯 1.3 + 角色冷蓝 tint/暖 add（灯池感）；卡侧 `night_mix=1`（整卡切夜版贴图）+ `night_comp=0.6`（EMISSION 补回烘卡亮度，见 §6.6）；后景窗火按 `BG_GLOW_RATIO` 0.45 折扣；夜雾删掉（层次交后景分层染色 + 远焦 DOF，§2.3）；夜空月盘 = 天幕弧线月盘（§太阳）。
...
- ~~CityGen plan.row≥1 运行时不消费~~ **已恢复（2026-09-17 创始人指认回归）**：中途一次背景改写把「三层背景+吃 plan.row≥1+地面跟第三层」丢成了「两层扫铺+地面跟第二层」——第三层整层消失、地面后退 3.5 格（=「后景缩水/没到最后一排根部」）。现恢复三层：bg1=plan.row≥1、bg2/bg3 吸附前层缝+补洞，末层楼根=地平线基线，far_z=get(2) 与它同源。
