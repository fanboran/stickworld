...
| `resource_manager.gd` | ✅ 已接入（：GameRoot 装配 + 建造扣资源 + ResourceBar UI；早期"代码完整但未接入"已解决。剩余 TODO：供需价格模拟，属 P1 经济深化） |

**GDD 要求**：
- ✅ 已实现：初始价格种子（`initial_price`）、库存/消耗/生产/跨区转移（含 5% 运输损耗）
- ❌ 未实现：供需自动调节价格（`_update_price`/`_tick_supply_demand` TODO，P1）
...
| `construction_manager.gd` | ✅ 已实现（阶段 0.4：construction_project + work_crew_assigner，tests/integration/test_construction_cycle 10/10 通过） |
...
| 文件 | 状态 |
|------|------|
| `modules/combat/api.gd` | ✅ 真实现（委托 BattleDirector，早期"api 空壳待清理"已解决，） |

**GDD 要求实现情况**：
- ✅ 已实现（阶段 0.5）：2D 侧视角/俯视角实时战斗渲染
- ✅ 已实现（阶段 0.5）：战术 AI（掩体利用、火力压制、侧翼包抄、交替掩护、撤退纪律、救助战友）
- ✅ 已实现（阶段 0.5）："小兵步枪"式灵动 AI（自主找掩体、劣势犹豫、追击过深）
- ✅ 已实现（0.7 装配， 16/16）：附身操控（WASD + 鼠标攻击），PossessionInterface 已装配
- 📌 规划延后：战斗结束判定（胜负收束/溃散判定/尸体处理）——依赖战斗与碰撞系统稳定后实施，不在 P0 范围
- ❌ 仍缺失：观察模式（看海）
- ✅ 已实现（阶段 0.5）：士气系统（battle_ai_director 情绪标签 + 溃散判定）
- ✅ 已实现（阶段 0.6）：指挥链延迟（command_chain + tactical_orders，P0 单层级零延迟链路已通）

### 3.2 logistics（物流模块）

| 文件 | 状态 |
|------|------|
| `modules/logistics/` | ❌ 模块已删除，仅剩 `config/logistics/carriers.tres`、`roads.tres` 两个数据文件；早期"api.gd 空壳"路径已失效 |

**GDD 要求但完全缺失**（P1 物流系统从零建）：
...
⚠️ 部分实现（阶段 0.6/0.7，）：已实现 BattlePanel（战斗面板）、Minimap（小地图）、PossessPanel（附身面板，已装配）、DebugOverlay（调试覆盖层）、GlobalHUD（居中模式按钮）。五层级主题与工作区预设仍未实现。
...
已消解：早期"`modules/` 仅 combat/logistics 空壳"的说法已不成立——`combat/api.gd` 已重写为 BattleDirector 委托式真实 API，`logistics` 目录已删除，`modules/` 下现为完整新模块。无待清理的旧空壳。
...
| ✅ 已接入基础资源管理循环 | consume/produce 已装配 GameRoot，建造扣资源已通 |
...
| 3 | ⚠️ 未完成 | `TechnologyState.gd` 逻辑改为征服获得（当前仍是研究进度驱动，P1 科技系统重建时一并处理） |
| 4 | ✅ 已完成 | 清理 `/modules/` 旧目录（combat api 已重写为真实现、logistics 已删除，） |
...
| 3 | ✅ 已接入 | 资源生产/消耗循环（GameRoot 装配 + 建造扣资源，早期"未装配"已解决） |
| 4 | ✅ 已完成 | 小队战斗基础实现（阶段 0.5：含战术 AI + 战斗结束判定） |
...
> 本版对照 复审，修正全部"已解决但文档滞后"的条目（资源接入/附身装配/combat api/双目录/logistics 删除）。
> **P0 编号说明**：早期本文档使用 P0-1=附身装配、P0-11=资源接入；与权威编号（[`场景与战斗架构.md`](../技术/架构/场景与战斗架构.md) §六，P0-1=Chunk 流式、P0-9=资源接入、P0-10/11/12=EnterTrigger/字段改名/战斗结束）不一致。**此后以架构文档 §六 编号为准**，本文档不再自创编号。

### 对照依据

| 依据 | 位置 | 说明 |
|------|------|------|
| P0 实施顺序与完成状态 | 场景与战斗架构.md §四 | 阶段 0.1-0.9 各项检查清单状态（✅ / ⚠️） |
| 已知问题与技术债 | 场景与战斗架构.md §六 | P0-1 ~ P0-17 共 17 项技术债 |

### 状态变更摘要

| 原标注 | 新标注 | 涉及条目 |
|--------|--------|----------|
| ⚠️ resources 未接入（P0-11） | ✅ 已接入 | §2.2、§六、§七#3 |
| ⚠️ combat api 空壳待清理 | ✅ 真实现 | §3.1 |
| ⚠️ 附身未装配（P0-1） | ✅ 已装配 16/16 | §3.1、§4.5 |
| ⚠️ 士气/指挥链部分实现 | ✅ 含 P0-12 战斗结束判定 | §3.1、§4.4 |
| ❌ logistics 空壳 | ❌ 模块已删除（数据文件保留） | §3.2 |
| ⚠️ 双模块目录待清理 | ✅ 已消解 | §5.1 |
| ⚠️ 资源循环未接入 | ✅ 已接入 | §七 原型最小可玩 #3 |

### 未变更条目（状态仍有效）

- §1.2 核心实体（StickmanState Race 枚举、TechnologyState 等待同步）
- §2.1 organization（树状拖拽/层级弹性/逐层指挥 AI/信息上报缺失；load_preset stub）
- §2.4 technology（仍为框架，manager 不存在）
- §3.3 achievement（仍无代码）
- §4.1 扩张系统、§4.2 法律系统、§4.3 无缝缩放（仍未实现）
- construction：建筑升级/修理仍 stub（P1）
