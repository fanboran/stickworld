...
##
## 渲染架构（方案 B · 矢量化描边，全局两遍渲染）：
## - 每段肢体 = 容器 Node2D（命名 sprite_<id> 保持扫描兼容），内含两层：
##   描边层（加宽深色圆头 Line2D）+ 填充层（同几何窄一层）。
## - 全局两遍：所有描边层 z=-1 压底、所有填充层 z=0 置顶 → 肢体重叠处
##   填充无缝融合，描边只在整体剪影外轮廓出线（"只有剪影描边"口径）。
##   部件间相对遮挡（外臂盖内臂、武器盖头）由填充层之间的树序承担
##   （reorder_render_order）。
...
## 全局两遍渲染：描边层 z=-1 压底、填充层 z=0 置顶——肢体重叠处填充
## 无缝融合，描边只在剪影外轮廓出线。
...
static func _build_limb(
	...
	if node_type == TYPE_CIRCLE:
		...
		fil.z_index = 0
		var stl := _make_line("stroke", pts, w + OUTLINE_WIDTH * 2.0, outline)
		stl.z_index = -1
