...
- **台基不烘**（创始人指认"建筑地面附近的灰白平面别烘进去"）：`buildings.py` 加 `PLINTH_ENABLED` 总开关，HD-2D 烘卡侧置 False（33 处调用点全部安全跳过），接触阴影踏面（flat_shadow_*）同步剥除（Blender 5.2 无 `Mesh.polygons.remove()`，删面走 bmesh），整楼按实测最低点（墙脚/台阶石）下沉贴地——墙原本砌在台基顶上，不沉会悬空；引擎 `base_cut` 裁剪退役（恒 0，旧 anchor 公式会误切真墙）；2D bake_export 管线默认不受影响
