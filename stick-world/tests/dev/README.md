...
> **ESC 退回主菜单**：debug 构建下任意 dev 场景（含界面模板陈列页）按 ESC 弹
> 确认框，确认后回主菜单，由 `DevSceneEscape` autoload 实现
> （`modules/debug_gui/scripts/dev_scene_escape.gd`）。本地 ESC 语义优先：
> 有可见 StickWindow 时先关窗、模态栈开着时先退栈；确认框开着时 ESC = 取消。
