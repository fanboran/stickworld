...
- **运行时切换**：`card.gdshader` 加 `albedo_night_tex` + `night_mix`（整卡切换，缺失回退日版）+ `night_comp`（EMISSion 补偿——夜间场景光按地面/角色需要给暗档，卡亮度不被二次压暗）；卡材质/道具/自然物装载夜版贴图
- **夜雾删掉**（创始人裁定）：后景/远处夜间"纯黑"的元凶是近黑雾色 + 后景吃满雾程；层次交给后景分层染色 + 远焦 DOF
- **窗光去黄**：glow 染色橙→暖白 `CARD_GLOW_TINT`、能量 1.15→0.7；街灯 (1.0,0.63,0.30)×1.6→(1.0,0.72,0.45)×1.3；后景卡登记 `_bg_card_mats` 参与夜窗光（`BG_GLOW_RATIO`=0.45 点缀档）
- **月亮盘光带修复**：夜间场景月亮能量 0.06→0.15、`sun_angle_max` 30°→2°——默认 30° 巨盘被远焦 DOF 糊成横跨天空的斜光带
- 验收：主街/算法村 昼夜四图实机核验（夜可读、后景有光、街具可见、白天零回归）；报错自检余 4 条为多开 Godot 锁 sqlite DLL 的环境噪音，与本批无关

### HD-2D 卡片管线修正批：台基退役 + 占地导出 + 宽度档规则（2026-09-15）

- **台基不烘**（创始人指认"建筑地面附近的灰白平面别烘进去"）：`buildings.py` 加 `PLINTH_ENABLED` 总开关，HD-2D 烘卡侧置 False（33 处调用点全部安全跳过），接触阴影踏面（flat_shadow_*）同步剥除（Blender 5.2 无 `Mesh.polygons.remove()`，删面走 bmesh），整楼按实测最低点（墙脚/台阶石）下沉贴地——墙原本砌在台基顶上，不沉会悬空。引擎 `base_cut` 改口径为 **alpha 扫描卡底透明留白**（恒 0 的过渡版让烘卡 PAD 留白被当地面、墙脚浮空 ~0.3 格，创始人二轮指认"浮空"；同 `_prop_bottom_pad` 口径），下沉后墙脚回到台基时代同一条基线，接地影 blob 不用动；2D bake_export 管线默认不受影响
- **地面占地随卡导出**（创始人问"有没有输出占地范围"——此前只有占地宽没有进深）：三套烘卡 meta 每条加 `footprint: [宽格, 深格]`，贴地顶点（z≤8px）实测、排除出檐悬挑
- **宽度档规则改 2 格步进**（创始人拍板，推翻 v3 唯一沿用旧约束"4 格整数倍"）：新增档位一律 2 格整数倍，存量 4/6/8/12/16 档保留不返工；v3 主规范 §3.1 尺度表已改

### 看板遮挡治理 + 运行图对齐 Frappe Gantt 惯例（2026-09-04）

- **程序化遮挡审计**：枚举全部 fixed/absolute 浮层矩形两两求交——坐实三项结构冲突后逐项归零
- **vbar 被顶栏压住**（用户指认"头栏被更上的栏遮挡"）：resize() 漏同步 vbar top（停在 95，顶栏长高到 105），
  且启动时 BAR_H 在图例填充前测量偏小——resize() 补 vbar 同步 + boot 阶段重测
- **运行图 v2（Frappe Gantt 惯例）**：任务条曾从 x=8 起画、直接压在泳道标签列上——左标签列 70px 独立于绘图区、
  顶部时间轴表头（刻度+单位说明）、条 3px 圆角；滚轮锚点/拖拽/命中检测全部感知新几何
- **状态条常驻**（VS Code 惯例）：侧栏 bottom:24px 不再盖住 cfoot 左右两段；小地图迁右下
  （React Flow MiniMap 默认 bottom-right），right:320 避开检查器栏
- **同层标注互叠治理**：墙标签（常驻结构）沉到墙线底部，顶部标注带只留聚拢标线/塔台横幅；
  悬浮文字一律加底衬 chip，任意画布内容上可读
- **agent 筛选改下拉**：21 个 agent 按钮横向溢出 dockBar → 原生 select（Grafana 变量筛选惯例）

### 看板视觉冲突与画风统一批次（2026-09-04）

- **agent 筛选按钮浏览器默认样式**：`.vt` 选择器误写成 `#vbar .vt`，dock 栏同名按钮完全没吃到主题样式
  （灰底 #6B6B6B、Arial、44px 高溢出 dockBar）——`.vt` 全局化后与 vbar 工具钮同语言
- **认领卡三重叠加**：顶部右上角标（◉ agent·时长）压在标题文字上、编辑●角标又压角标——删除顶部角标，
  认领标识收敛为底部胶囊（agent 色 + 认领时长）+ 边框呼吸辉光；选中态改外扩粉环，与认领内框并存（曾内框被吞）
- **画布标注被工具条遮挡**：塔台横幅/聚拢前沿·后继标线/分割墙标签的 y 都在 vbar 带内，上半被 DOM 工具条盖住——统一下移
- **画风统一**：vbar 底色 `--line`→`--panel`（与侧栏一体的 IDE 底）；浮层族（ctx/modal/relwin/help）统一
  6px 圆角 + line2 描边 + 同款投影（此前 ctx 直角 4px、relwin 独有阴影、注释自相矛盾）；调度 chips 圆角 5→4；
  两画布底色/网格色统一；运行图泳道标签列 44→64px（中文 4-5 字完整可读）；树注超宽不再冲出卡片
