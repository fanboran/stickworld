...
### HD-2D 样板街当前状态与遗留（2026-09-14 凌晨，场景多轮迭代后）

**当前基线**：`proto_hd2d.gd/.tscn` = 初版 `6908836d` + 台面结构 + 排布算法（HEAD `7f0b4ee3` 附近）。场景为**运行时代码搭建**（`proto_hd2d.gd`），编辑器里直接打开 `.tscn` 看不到内容，须运行或用 `--save-scene` 导出。

**已实现（当前渲染可见）**
- 地面 = **两种材质两段**：台面（人行道，暖亮浅石，`band_shoulder_stone` HD+法线，从真实地平线 z=−6.5 铺到台肩 z=1.95）+ 道路（冷深石，`band_road_stone`）；台肩前沿 = **方形截面长石镶边**（高=深=台面高 0.65 格，顶面与台面齐平压低 0.01 防 z-fighting）
- **前排一格挨一格铺满整条可见街**（9 种混排循环：cottage/house/shop/bakery/townhouse/guildhall/smithy/tavern/stable），落在台面上（y +PLAT_H）
- **三层背景**（z=−9.5/−12.5/−15.5）从缝隙算法生成、近小远大（远层限宽 ≤12 格），逐层虚化，**把真实地平线遮死**（底衬远端 = 末层根部 = z −15.5）
- 俯角 **26°**（创始人要求从 20° 加）、相机 `CAM_CY=11`/`CAM_DIST=40`/正交视宽 74 格（25.9px/格）
- 火柴人纯黑剪影是**设计**（创始人确认），不改

**创始人看过最新版后仍判"不对"（未解决，下一会话第一优先）**
- 台面上方（长石上）的地面色与下方道路仍分不开——台面材质要换**非灰系**（如暖沙岩/夯土色）或更明显的色相区分
- 前排建筑排布观感仍未达"闹市"密度感；背景层覆盖与虚实分层需按创始人逐张图再对
- 建筑离台肩的距离变化（有的近有的退）在最新版是否体现待核
- 其余待裁项见 `docs/项目/待办事项.md`「需创始人决策」（门宽/窗宽规范冲突、俯角 vs 屏占、零偏航侧翼、B 类室内清单）

**可调参数速查**（都在 `proto_hd2d.gd`）：`PLAT_H`（台面高 0.65）、`TILT_DEG`（26）、`CAM_CY`（11）、`CAM_W`（74）、`BAND_SIDEWALK`（0.42~1.95）、`SKYLINE_Z`（−9.5，按 26° 投影公式解得：基线正好压 1/3 线）、台面 tint（暖亮 1.04/1.00/0.93）与道路 tint（冷深 0.86/0.89/0.96）
