...
- **夜版卡（月光烘进 Blender）**：三套烘卡脚本（blender_proto/bake_props/bake_nature）每卡多烘一张 `<卡>_night.png`——亮冷蓝月亮方向光（正面高角度斜打）+ 深蓝夜环境，窗/火/水晶自发光烘进 albedo；建筑 50 + 道具 48 + 自然物 15 全量入库 `tex/`（113 张）。修过两轮：月亮首版从背面打光致正立面全黑、窗自发光 1.9 直冲 8bit 上限吹成白块，定稿（55°,-75°）/0.92 档
