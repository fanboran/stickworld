...
- **斜线去锯齿（DP 折线简化）**：像素蒙版轮廓的 45° 楼梯（1px 一格台阶，特写 ~4.6px/格）用 Douglas-Peucker(tol=1.0) 折成直线——短边(<1px)残留从 100% 降到 ~3%，产出 29 条干净 45° 斜段；同源同参数共享边仍无缝；修复 split_self_touch 自触碎条被 `outs[0]` 误取（city_1034 曾只剩 3 点碎三角）——改取面积最大 outer 作主轮廓、剔除 <3px² 碎条
