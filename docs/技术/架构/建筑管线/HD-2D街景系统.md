...
- 地面三带贴图（2026-09-15 定稿）：背景地面带（兜底）只铺到**第二排后景根部**（远端不拉太高，地平线=第二排楼脚——创始人 2026-09-15），贴图=稀疏草土（与城外野地统一）；末排楼脚以后就是天空（昼档纯蓝）。
...
### 4.0 线名对照（术语校准，创始人 2026-09-15）

旧文档多把"地平线"用在 1/3 那条线上——术语自本节起校准，读旧文档时按下表对号：

| 线 | 世界线 | 屏幕位置（zoom=1） | 旧称/易混名 |
|---|---|---|---|
| **前后景分界线** | SKYLINE_Z=-6.73 格 ↔ 2D y≈472.6 | 屏幕下 1/3 线（720） | 旧文档"地平线"、天际线基线 |
| **新地平线**（真实地平线） | 末排背景基线（底衬远端收于此，见 §三点五） | 高于 1/3 线（更远） | —— |
| **屏幕底沿锚线** | z_near≈18.93 格 ↔ 2D y=1294（WALK_FRONT_Y） | 屏幕底沿 | —— |
| **F3 黄线** | HD-2D 图 = 前后景分界线；2D 图 = ground_y | 720 | ground_y 线 |
...
- **建筑宽度辅助线**（2D 图 F3 版式样原样搬入，白色口径，创始人 2026-09-15）：
  ...
  - ⚠ 占地带四元组是**混合口径：x=格（×32 转 px）、y=px**——与 get_solid_rects
    碰撞墙消费端同源同口径，画前先换算 x。
...
| bg2 基线 = 真实地平线 | far_z ≈ −14.2（= SKYLINE_Z − 层距；层距=等距口径 ≈7.4——创始人：第二三排距离=第一二排距离） | 末排楼脚；远景地面带/城墙/草地带全部收于此线 |

### 6.3 分层解剖（z 从远到近；遮挡由深度测试天然排序）

| # | 层 | z | 要点 |
|---|---|---|---|
| 1 | 程序化天空 | — | ProceduralSkyMaterial **纯色天空**（昼=纯蓝一档，上下半球同色无亮带）+ **太阳盘 = 对 `_sun` 方向光的自动渲染**（右上，昼显夜隐，被楼群遮挡由深度测试天然处理，无独立节点）；**漂移云牌** = 2D 手绘云（SketchCloud）烘贴图的 Sprite3D billboard（14 朵池、风驱漂移+出带回绕、昼白/夜暗蓝，z 钉地平线身后）；昼/夜色值都在 `_apply_light` |
| 2 | 远景地面带 | far_z→0 | 城心石板（±30）/ 夯土（±30→±墙线）/ 城外草地（±墙线→±(墙线+30)，贯通 far_z→46）；接住背景楼脚 |
| 3 | 兜底大地皮 | far_z→40 | 1200 格宽夯土，y=−0.05 压在所有分段之下，防任意缩放露底；**远端收在第二排后景基线**（=真实地平线，"地平线=第二排楼脚"，§4.0 新地平线） |
| 4 | 背景第二排 bg2（地平线补缝） | ≈−14.2 | **稀疏补缝**：只插在 bg1 露出的地平线缝隙里（一缝一栋、中心整格），楼身站上真实地平线、穿缝可见、楼顶可越前排露出；tint 最淡最冷档（空气透视）；满档远焦模糊；剪影档渲染（不投影、不进夜景窗火） |
| 5 | 背景第一排 bg1（主天际线） | −6.73 | 整排连铺钉 1/3 分界线（整格摆位、楼间缝 2~4 整格）；tint 浓一档；半档模糊（DOF 起点=分界线基线） |
| 6 | 城墙 | far_z→19.5 | ±墙线（布局半宽推导，手摆兜底 95）；厚 1.2、高 10 格（town 档 320px）；**纵深贯通到后景地平线**；门洞带 z 8~14（两段墙板夹洞 + 门柱加厚/叠涩内挑/横梁组成门楼），垛口步距 1.7 格 |
| 7 | 前排建筑 row0 | 0.45~2.4 | **中心 x 吸附整格**（宽度=格整数倍，间隙承诺由整格步距兑现）；台面带 z 0.42~1.95 垫高 PLAT_H=0.65；卡底贴地落位（可视墙脚=z_off）；z 错落 0.45~1.3=台面 / 2.4=落地面（barn/cottage 类）；烘焙台基带 base_cut 整段裁掉；贴地程序化接地影（z 3.3~5.9——卡内烘焙接触影在卡深度面上会被路肩盖掉，必须补才能"钉"在路肩上）；door 建筑加门前短径（楼脚→台肩→路面） |
...
- **bg1 主天际线=连续扫铺**：从前排跨度两端各收 3 格起逐张顺铺，全卡池随机（只排除附近已用卡——创始人：选卡自由度最高），**中心吸附整格 + 楼间缝 2~3 整格**（取整余数最多再 +1，实际缝 2~4 格），末卡越界超 2 格收边不出墙；种子按（卡位 x, 层）确定性 → 多局一致。
...
- **day = 阳光明媚高调照明**：**天空纯蓝一档**（上下半球同色，无地平亮带；太阳盘照常渲染）+ 主光暖白 0.48（太阳盘右上）+ 冷天空环境 0.56（两光和≈1.0——卡是烘焙图，光照别双计）+ 冷补光 0.12 抬暗部（明暗比≈1.15:1）；雾关；tonemap LINEAR（filmic/aces 会把烘焙卡压灰）。
