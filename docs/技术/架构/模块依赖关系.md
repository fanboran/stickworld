...
**关键路径**：`GameRoot`（装配）→ `player_control`（输入）→ `combat`/`construction`/`resources`（玩法系统）→ `units`（执行）→ `world`（容器）。`organization` 是设计中的枢纽，实际仅被 `FormationSystem` 用于创建 L1 组织节点。科技（technology）模块已删除，阶段 1 按「征服获得」策略重建。
...
| **Technology** | 已删除（原 `modules/technology/api.gd` 空壳） | ❌ 已删 | 阶段 1 按 04-科技系统.md「征服获得」策略重建模块 |
...
| **combat** | 战斗实例+编队+号令+掩体+导演 | `battle_instance.gd`、`battle_director.gd`、`formation_system.gd`、`tactical_orders.gd`、`command_chain.gd`、`cover_system.gd`、`battle_ai_director.gd` | units（实体）、organization（L1 组织节点）、config/formations（编制预设） | GameRoot、ui_global（BattlePanel 在 combat/ui）、player_control |
...
| **player_control** | 输入分发/框选/附身 | `input_dispatcher.gd`、`explore_handler.gd`、`selection_system.gd`（combat/）、`possession_interface.gd` | units、combat、world | GameRoot、ui_global |
...
| 模块 | 状态 |
|---|---|
| **technology** | 模块已删除（原空壳），阶段 1 按「征服获得」策略重建（见待办事项.md） |
...
| # | 断裂 | 表现 | 修复前置 |
|---|---|---|---|
| 1 | **technology 模块缺失** | 科技系统全缺（原空壳模块已删除） | 阶段 1：按 04-科技系统.md 建模块（征服制） |
