...
# 根因（烘焙图查实）：出檐 = 建筑宽 × 20.5%/侧（`buildings.eave_over`），自 2026-09-17
#   起加**绝对封顶 60**（≥10 格宽的楼檐长恒 1.875 格），可见剪影宽 ≈ 占地宽 + 3.75 格
#   （小建筑仍 ≈ ×1.4，见 `sil_w_cells`）。相邻两栋若按**占地框**排布，两侧出檐必然互压
...
#     对数与缺口格数供裁决。
EAVE_RATIO = 0.205
#: 出檐绝对封顶（折格）：buildings.py EAVE_ABS_MAX=60 / 32——≥10 格宽的楼檐长不再随宽度增长
EAVE_ABS_CAP_CELLS = 60.0 / 32.0
...
def eave_cells(w_cells: int) -> int:
    ...
    return min(int(int(w_cells * CELL_W * EAVE_RATIO + 0.5) / float(CELL_W) + 0.5),
               EAVE_ABS_CAP_CELLS)
