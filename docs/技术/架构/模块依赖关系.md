...
> **装配根说明**：`world` 模块承载 `GameRoot`/`system_setup.gd`（composition root），负责装配全部子系统。因此 `world → 各模块` 是装配依赖；`combat → world` 仅剩**注入路径常量**（`battle_anchor_path`/`building_host_path`，由装配层注入），**world⇄combat 为有意环，勿再新增模块对 world 的内部引用**。
> 2026-09-11 audit_deps 实测更新：另存在 **world⇄world_map 环**（world_map 经全局类 WorldAPI 消费 MapType/EntrySide/PATH_* 等共享词汇，preload 层面 world→world_map 仅装配器一处）——拆解方向 = 共享词汇下沉 `core/shared/`（AR-2，见 `项目/待办事项.md`）；system_setup 跨模块 preload 41 处/12 模块（AR-1 装配下沉跟踪）。本图主体刷新随 AR-2 执行（当前图缺 inventory、world 层级画法过时）。

```mermaid
flowchart TD
    GR["GameRoot（常驻主场景）<br/>game_root.tscn + system_setup.gd 装配一切"]

    GR --> UI["ui_global（三层容器/P0✅）"]
    GR --> DBG["debug_gui（P0✅）"]
    GR --> PC["player_control<br/>（输入分发/附身/P0✅）"]
    GR --> WM["world_map（战略图）<br/>L1 单层已实装（Tab）+ L3/L2 视图（M）"]
...
**关键路径**：`GameRoot`（装配）→ `player_control`（输入）→ `combat`/`construction`/`resources`（玩法系统）→ `units`（执行）→ `world`（容器）。`organization` 是设计中的枢纽，实际仅被 `FormationSystem` 用于创建 L1 组织节点。科技系统当前无模块，阶段 1 按「征服获得」策略重建。
...
| **Technology** | 无模块 | ⬜ 未实现 | 阶段 1 按 04-科技系统.md「征服获得」策略重建模块 |
...
|---|---|---|---|---|
| **world** | 场景图容器/相机/加载 | `world/scripts/game_root.gd`、`scene_loader.gd`、`hd2d_street_map.gd` | units（spawn 实体）、construction（注入） | 所有模块（装配中枢） |
...
| **combat** | 战斗实例+编队+号令+掩体+导演 | `battle_instance.gd`、`battle_director.gd`、`selection_system.gd`、`formation_system.gd`、`tactical_orders.gd`、`command_chain.gd`、`cover_system.gd`、`battle_ai_director.gd` | units（实体）、organization（L1 组织节点）、config/formations（编制预设） | GameRoot、ui_global（BattlePanel 在 combat/ui）、player_control |
...
| **player_control** | 输入分发/附身 | `input_dispatcher.gd`、`explore_handler.gd`、`possession_interface.gd` | units、combat（框选/编队/战术指令在 combat/scripts/command/）、world | GameRoot、ui_global |
| **ui_global** | 三层 UI 容器 | `ui_root.gd`、`mode_panel.gd`、`minimap.gd`（业务面板在 combat/ui 等模块） | 各玩法系统 api（setup 注入） | GameRoot |
...
| **world_map（战略图）** | L1 单层战略图（Tab）+ L3/L2 下钻视图（M）已实装：索引图命中/边界描边/空聚落/进出链路，集成测试 3 套全绿 | L1 地块划分（tools/worldgen 生成管线）未开始；8 城邦数据当前为手写占位 |

### 3.3 空壳/未创建（⬜）
| 模块 | 状态 |
|---|---|
| **technology** | 无模块，阶段 1 按「征服获得」策略重建（见待办事项.md） |
...
| # | 断裂 | 表现 | 修复前置 |
|---|---|---|---|
| 1 | **technology 模块缺失** | 科技系统全缺 | 阶段 1：按 04-科技系统.md 建模块（征服制） |
| 2 | **数值数据驱动断裂**：config/units/*.tres ↔ StickmanEntity | 兵种无差异、平衡表失效 | BalanceConfig autoload 已存在（读取器就绪），需接入实体 + 战斗组件 |
| 3 | **战斗无可感知性**：伤害链路 ✅ 但无 UI 反馈 | 血条/伤害数字/受击表现缺失 | ui 模块 + visual_controller 扩展 |
| 4 | **室内断裂**：building def interior_mode=0、无室内内容、零测试（旧 mega_interior 传送链已随 2D 图清退拆除，室内由建筑管线 v3 内景承担） | 原型循环验收不含"室内"环节 | 随建筑管线 v3 内景接线深化 |
| 5 | **resources 供需未接入** | 经济系统无循环 | `_update_price`/`_tick_supply_demand` TODO 实现 + 生产建筑接入 |
| 6 | **组织不驱动行为** | 组织只是数据容器，不自动派活 | 阶段 1 组织系统深化（load_preset/任务分解） |
| 7 | **跨图携带只覆盖编队** | NPC 村民不随行（设计如此 P0）；存档不含编队 | 存档扩展（P1） |

---

## 七、测试套件 ↔ 验证链路（证据索引）

> 全量运行（bash）：`bash tests/run_all.sh`（unit 批量 9 套 + integration 22 + smoke 2，2026-08 审计校准后默认并行 4 + 每套件超时表，实测全绿 24/24）
...
| unit/（9 套件） | unit | 各系统纯逻辑：Health/资源/指挥链/编队/状态机/网格/PlacementGrid |
| test_strategic_map_p0 / test_l2_strategic_map / test_l3_strategic_map | integration | world_map：L1 索引命中/边界/进出 + L2 下钻 + L3 视图 |

**未被任何测试覆盖**：resources 供需、室内、数值数据驱动、血条视觉。
