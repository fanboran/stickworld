...
> **编号约定**：本方案批次一律 `UI-W<n>` 前缀——项目里另有一套 WorldBox 机制批次用 `W1~W10`（含 `WB<n>` 别名，见 [`../../设计/系统/12-游戏AI系统.md`](../系统/12-游戏AI系统.md) §2.4），裸用 `W1` 会两边撞号。批次状态与进度以 [`../../项目/交接/游戏AI集大成-进度与交接.md`](../../项目/交接/游戏AI集大成-进度与交接.md) 为准（本表只描述范围与验收门）。

| 批次 | 内容 | 档位 | 验收门 |
|------|------|------|--------|
| **UI-W1 观测接线批**（调试向） | §2.5 TeamAi 生产启用接线 + §2.2 TeamAi HUD + §2.1 悬停增强 + §2.6 前三行接口补齐（相位信号/attack% getter/撤退调制 getter） | 小 | check_godot_errors 干净；run_all 零回归；F3 开关族逐项可控显隐 |
| **UI-W2 组织界面 MVP** | §3.2.A L1 班组卡（ContextPanel 槽位）+ OrgPanel 树增强（节点状态徽标/「群龙无首」标记）+ 上报流消费（toast 级） | 中 | 同上 + ui_shots 截图自检（[09-布局规则与AI自检](09-布局规则与AI自检.md) 惯例）+ 创始人观感初验 |
| **UI-W3 指挥链可视化** | §2.6 末行在途命令注册表 + §3.2.B 命令传播动画层 + 每跳 ETA 标注 + 停驻表达 | 大 | 同上 + 传播动画与 transport 实测延迟一致性抽检 |
| **UI-W4 逐层深化** | §3.2.B 中间层节点牌完善（补位候选序/统辖规模）+ §3.2.C 战略总览面板 + §3.3① 权威值择班表达 + 图标缺口母题立项 | 中~大 | 同上 + 全层级视图观感走查（裁决②基调） |
