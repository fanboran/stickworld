...
# ------------------------------------------------------------ 做旧层 / 逐体色变
#
# 做旧层（AGE 表 + `_age_wall`）**现行下线**（AGE_OFF=True）：观感基准 =
# **明亮干净**——实机里溅泥/苔/雨渍读不出"岁月感"，只剩脏感（创始人裁决）。
# 表与实现保留作登记，重开做旧只需改 AGE_OFF。
# 逐体色变（OBJ_VAR）继续生效，治"所有建筑一个色"（材质实例按名字复用 →
# 全局共享一个 Tint）；幅度必须在 25% 门禁尺寸下仍成立 → 只走低频。
#
# 分工纪律：
...
#: 做旧层总开关：现行观感基准 = **明亮干净**（创始人裁决：实机读不出岁月感、
#: 只剩脏感）→ `_age_wall` 一律直通，AGE 表保留作参数登记；重开做旧改 False。
AGE_OFF = True
...
def _age_wall(b, u, v, col, key):
    ...
    `AGE_OFF=True` 时直通（现行设计：观感基准明亮干净，做旧层下线）。
    开启时的顺序：日照褪色（大尺度底色不匀）→ 近地溅泥/苔 → 垂直雨渍
    （≤6% 扰动）。三层都低频克制，保证 25% 门禁下是"读得出材质+有年头"。
    """
    if AGE_OFF:
        return col
    cfg = AGE.get(key)
