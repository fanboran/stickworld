...
| 属性 | 类型 | 说明 |
|------|------|------|
| `Weapon Scene` | PackedScene | 右手武器。运行时由 `weapon_mount.gd` 的 `WEAPON_SCENE_PATHS` 按 `WeaponType` 装配（剑/矛/弓/镐/杖各自场景），Inspector 直接赋值可覆盖 |
| `Offhand Scene` | PackedScene | 左手武器（盾/副武器），留空表示空手 |
...
```gdscript
# 装备
$StickmanRig.weapon_scene = load("res://modules/units/scenes/components/weapon_sword.tscn")

# 换武器
$StickmanRig.weapon_scene = load("res://modules/units/scenes/components/weapon_sword.tscn")

# 双持
$StickmanRig.offhand_scene = load("res://modules/units/scenes/components/weapon_shield.tscn")  # 副手：盾/法杖等
