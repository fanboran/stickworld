...
### L1 细分做进 L2 下钻视图（2026-08，点开 L2 看到 L1 细分）

- **生成端**：新增 `l1/export_l1_l2_split.py`——把全大陆 L1 蒙版按地区切片为 `l2_packs/region_XXX/l1_split.json`（context 局部坐标，与 l2_world.json 同一坐标系；13 地区共导出 389 个 L1 细胞）
- **消费端**：L2 下钻视图（点开地区）默认显示该地区 L1 细分——细胞蒙版相似色半透明填充 + 边界线 + 城市点（`l2_l1_overlay.gd`，ArrayMesh 静态烘焙零逐帧剖分）；V 键切换回旧地块视图；细分视图下隐藏旧地块色块/描边/hover
- **测试**：新增 `test_l2_l1_overlay`（open 默认细分/几何合法/V 切换）3/3 绿；战略图相关 5 套件回归全绿
- **文档**：08-程序化世界生成 §L3-L 增补 L2 下钻查看
