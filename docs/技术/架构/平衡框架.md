...
> **注意**：变量来源已迁移到 Excel 管线。原始 `.xlsx` 文件位于 `config/excel/`，导出目标为 `config/*.tres`。数据表的编辑、导出流程详见 [excel-pipeline.md](../教程/Excel数据管线.md)。本文档中描述的 `.tres` 手动编写方式已被 Excel 管线取代。
...
```gdscript
# 示例：modules/combat/scripts/battle/battle_instance.gd（实际伤害计算在 WeaponMount/HealthComponent）
