...
- 当前活跃交接文档：
  ...
  - `docs/项目/交接/火柴人视觉修复-进度与交接.md`（任务：火柴人逆向重建——从 Stick War 解包真值数据重建骨架/动画/渲染；**批次 1-4h、4i-A（导入器）、4i-B（渲染重标：56 骨真值骨架+附件几何矢量肢体+动画切真 Spine 数据）全部完成，视觉 judge PASS、D1~D11 实质清零；批次 C（数值 RMS 验收）/D（画廊目检终验）待做**；注意 4i 工作实际在分支 `agent/conquest-loop`、worktree `.temp/conquest-loop` 内推进，`.temp/stickman-visual` 仅存批次 1-4）
