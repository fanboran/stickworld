...
func _format_cost(def: Dictionary) -> String:
	var parts: Array = []
	var map := {"build_cost_wood": "木", "build_cost_stone": "石", "build_cost_metal": "铁"}
	for key in map.keys():
		var v: float = float(def.get(key, 0))
		if v > 0:
			parts.append("%s:%d" % [map[key], int(v)])
	if parts.is_empty():
		return "免费"
	return " · ".join(parts)
...
func _can_afford(def: Dictionary) -> bool:
	if _resources_api == null:
		return true
	var keys := ["build_cost_wood", "build_cost_stone", "build_cost_metal"]
	for key in keys:
		var v: float = float(def.get(key, 0))
		if v <= 0:
			continue
		var res_id: String = "res_" + key.replace("build_cost_", "")
...
func _start_placing(def_id: String) -> void:
	...
	print("[BuildMenu] 进入选址模式: %s" % def_id)
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.pressed:
		if event.button_index == MOUSE_BUTTON_LEFT:
			print("[BuildMenu] 收到左键 -> 确认建造")
			_confirm_place()
			get_viewport().set_input_as_handled()
		elif event.button_index == MOUSE_BUTTON_RIGHT:
			print("[BuildMenu] 收到右键 -> 取消选址")
			_cancel_placing()
			get_viewport().set_input_as_handled()
	elif event is InputEventKey and event.pressed:
		if event.keycode == KEY_ESCAPE:
			print("[BuildMenu] 收到 ESC -> 取消选址")
			_cancel_placing()
			get_viewport().set_input_as_handled()
