...
# ─────────────────────────────── 常量 ────────────────────────────────
## 工作类型选项（id 引用 FormationSystem.WorkType，避免字符串散落）
const WORK_OPTIONS: Array = [
	{"label": "战斗", "id": FormationSystem.WorkType.COMBAT},
	{"label": "建造", "id": FormationSystem.WorkType.BUILD},
	{"label": "搬运", "id": FormationSystem.WorkType.HAUL},
	{"label": "采集", "id": FormationSystem.WorkType.FORAGE},
]
...
func _refresh_detail() -> void:
	...
	for i in range(members.size()):
		...
		var role_str: String = _role_zh(u.get_role() if u.has_method("get_role") else "")
		var is_leader: bool = (_formation.get_squad_leader(sid) == u) if _formation.has_method("get_squad_leader") else false
		u_label.text = "成员%d（%s）%s" % [i + 1, role_str, "★排长" if is_leader else ""]
...
func _refresh_idle_list() -> void:
	...
	for e in map.get_entities():
		...
		count += 1
		var cb := CheckBox.new()
		var role_str: String = _role_zh(e.get_role() if e.has_method("get_role") else "")
		cb.text = "%s%d" % [role_str, count]
...
## 角色英文 id → 中文显示名（fighter/builder/worker）
func _role_zh(role: String) -> String:
	match role:
		"fighter": return "战士"
		"builder": return "建造工"
		"worker": return "工人"
