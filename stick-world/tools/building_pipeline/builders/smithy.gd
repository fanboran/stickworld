...
static func _tier1(p: Painter, L: Dictionary) -> void:
	...
	var rng := p.branch("s1")
	# 后景地面（棚内浅色地坪）
	p.poly(Comps._rect_pts(Rect2(6.0, wall_top - 26.0, px_w - 12.0, baseline - wall_top + 26.0)), Color(0.86, 0.79, 0.66))
	# 柱（前柱醒目，后柱略暗）
	var n := maxi(3, roundi(px_w / 96.0) + 1)
	for i in n:
	...
	# 柱顶横梁
	p.stroke(PackedVector2Array([Vector2(6, wall_top + 3), Vector2(px_w - 6, wall_top + 3)]), 4.4, {"rng": rng, "amp": 0.5})
	# 斜撑（两端人字撑）
	p.stroke(PackedVector2Array([Vector2(18, wall_top + 3), Vector2(px_w * 0.5, wall_top - 30)]), 3.0, {"rng": rng, "amp": 0.5})
	p.stroke(PackedVector2Array([Vector2(px_w - 18, wall_top + 3), Vector2(px_w * 0.5, wall_top - 30)]), 3.0, {"rng": rng, "amp": 0.5})
	# 茅草双坡顶（山墙朝前）
	var roof := {"rise": 44.0, "overhang": 16.0, "mat": "thatch"}
	Comps.gable_roof(p, Rect2(6.0, wall_top, px_w - 12.0, 10.0), roof, rng, false)
...
static func _tier2(p: Painter, L: Dictionary) -> void:
	...
	# 右侧大窗（十字棂）
	Comps.window(p, Rect2(px_w * 0.62, wall_top + float(L["wall_h"]) * 0.3, 26.0, 24.0), p.branch("s2_win"), false)
	# 门（右侧）
	var dr := Rect2(px_w * 0.88, baseline - float(L["wall_h"]) * 0.66, 26.0, float(L["wall_h"]) * 0.66)
	Comps.door(p, dr, "wood_dark", p.branch("s2_door"))
	# 木板双坡顶（山墙朝前，山墙窗）
	var roof := {"rise": 50.0, "overhang": 10.0, "mat": "roof_wood"}
...
static func _tier3(p: Painter, L: Dictionary) -> void:
	...
	Comps.arch_window(p, Rect2(px_w * 0.3, wall_top + wall_h * 0.06, 20.0, 22.0), p.branch("s3_upw1"), false)
	...
	p.rect_stroke(open_rect, 2.4, {"rng": rng, "amp": 0.6})
	# 右拱窗 + 拱门
	Comps.arch_window(p, Rect2(px_w * 0.66, baseline - wall_h * 0.5, 30.0, 34.0), p.branch("s3_awin"), false)
	# 平缓石瓦顶
	Comps.slope_roof(p, body, {"rise": 24.0, "eave_drop": 9.0, "skew": 8.0, "overhang": 11.0, "mat": "slate"}, p.branch("s3_roof"))
...
static func _tier4(p: Painter, L: Dictionary) -> void:
	...
	for sx in [body.position.x + 3.0, body.end.x - 9.0]:
		var pr := Rect2(float(sx), wall_top + 4.0, 6.0, wall_h - 6.0)
		p.face(pr, "trim_white", "base")
		p.rect_stroke(pr, 1.8, {"rng": rng, "amp": 0.4})
	# 层间白石饰带
	var belt := Rect2(body.position.x + 2.0, wall_top + wall_h * 0.42, body.size.x - 4.0, 7.0)
	p.face(belt, "trim_white", "base")
	p.rect_stroke(belt, 1.8, {"rng": rng, "amp": 0.4})
	# 拱窗（上部）
	Comps.arch_window(p, Rect2(px_w * 0.24, wall_top + wall_h * 0.5, 26.0, 30.0), p.branch("s4_w1"), false)
	...
	var top_l := Vector2(body.position.x - 2.0, eave_y - 26.0)
	var top_r := Vector2(body.end.x + 2.0, eave_y - 26.0)
	...
	for k in 2:
		var cx := body.get_center().x + (-1.0 if k == 0 else 1.0) * px_w * 0.16
		var cy := wall_top - wall_h * 0.06
		var circ := PackedVector2Array()
		for i in 8:
			var a := TAU * float(i) / 8.0
			circ.push_back(Vector2(cx + cos(a) * 6.0, cy + sin(a) * 6.0))
		p.stroke(circ, 1.8, {"rng": rng, "amp": 0.3, "closed": true, "overshoot": 0.0, "taper": false})
	with_roof(p, L, rng)
...
static func with_roof(p: Painter, L: Dictionary, rng: RandomNumberGenerator) -> void:
	var body: Rect2 = L["body"]
	Comps.slope_roof(p, Rect2(body.position.x + 8.0, body.position.y - 20.0, body.size.x - 16.0, 20.0), {"rise": 22.0, "eave_drop": 6.0, "skew": 4.0, "overhang": 8.0, "mat": "tile"}, rng)
...
static func _props(p: Painter, L: Dictionary, meta: Dictionary) -> void:
	...
	Comps.anvil(p, Vector2(anvil_x, baseline - 2.0), 26.0, p.branch("anvil"))
	Comps.barrel(p, Vector2(px_w * 0.08, baseline - 2.0), 24.0, p.branch("barrel"))
	Comps.bench(p, Vector2(px_w * (0.86 if tier != "smithy1" else 0.62), baseline - 2.0), 28.0, p.branch("bench"))
	meta["workslots"] = [
