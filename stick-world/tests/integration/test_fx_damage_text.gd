...
##
## 测试覆盖：
##   - 基准表现：zoom=1 普通伤 12 号/描边 4（c6919002 基准减半）；文本取整；弹性起手放大
##   - 恒定屏上尺寸：拉远反向放大（zoom 0.5→24）+ 上下钳制（0.35→28 / 3.0→11）
##   - 暴击：34 号/描边 8 + 歪斜起手回正
...
func _register_tests() -> void:
	_tests.append({"name": "飘字: 基准表现（zoom=1 普通 12/描边4/弹性起手）", "fn": Callable(self, "_test_base"), "async": true})
	_tests.append({"name": "飘字: 拉远反向放大 zoom 0.5→24", "fn": Callable(self, "_test_zoom_out"), "async": true})
...
func _test_base() -> void:
	...
	_runner.assert_equal(lb.get_theme_font_size("font_size"), 12, "zoom=1 普通伤害字号 12（c6919002 基准减半）")
	_runner.assert_equal(lb.get_theme_constant("outline_size"), 4, "描边 = clampi(字号/4, 4, 12) = 4")
...
func _test_zoom_out() -> void:
	...
	if lb == null:
		...
		return
	_runner.assert_equal(lb.get_theme_font_size("font_size"), 24, "zoom=0.5 应反向放大到 24")
...
func _test_clamp() -> void:
	...
	if lb == null:
		...
		return
	_runner.assert_equal(lb.get_theme_font_size("font_size"), 28, "zoom=0.35 应钳制在 28 上限")
	await _settle()
	...
	if lb == null:
		...
		return
	_runner.assert_equal(lb.get_theme_font_size("font_size"), 11, "zoom=3.0 应钳制在 11 下限")
...
func _test_crit() -> void:
	...
	if lb == null:
		...
		return
	_runner.assert_equal(lb.get_theme_font_size("font_size"), 17, "暴击字号 17（基准减半）")
	_runner.assert_equal(lb.get_theme_constant("outline_size"), 4, "暴击描边 clampi(17/4, 4, 12) = 4")
