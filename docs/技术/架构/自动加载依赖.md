...
>
> 当前 7 个 Autoload：EventBus / ConfigManager / SaveManager / SceneManager / AudioManager / DebugApi（`modules/debug_GUI/api.gd`，2026-07-27 改名前叫 `modules/debug/`） / `_mcp_game_helper`。
...
> 现状：以上四项**均未实现**。README §10 备注中有简略说明，本节保留完整代码示例作为实现参考。
