...
func setup(org_api: Node) -> void:
	...
	# A9+ 权威值自主跳槽参数装载（缺省关闭，见文件尾 §权威值自主跳槽 段）
	_load_authority_params()
...
func _process(_delta: float) -> void:
	...
	# A9+ 权威值自主跳槽（R4）：按 L2 节拍周期性评估成员择班（缺省关闭时零开销空转）
...
## 权威值经济定"）。本段 = 评分内核与滞回判定；成员自主跳槽行为（周期性评估 +
## 冷却 / 错峰 / 守卫）见文件尾 §权威值自主跳槽——行为侧复用同一套权威值与
## authority_margin 判定，只补调度与守卫，不重写比较逻辑。
...
const AUTHORITY_DEFAULTS: Dictionary = {
	...
	"authority_eval_interval": 2.0,
	"authority_switch_cooldown": 20.0,
	"authority_candidate_radius": 800.0,
	"authority_player_squad_bonus": 0.2,
	"authority_stay_in_player_squad_bonus": 0.5,
	"authority_leader_release_threshold": 0.3,
	"authority_max_switches_per_squad_tick": 1,
	"authority_rng_seed": 20260913,
}

## 跳槽参数（AUTHORITY_DEFAULTS ← config/ai/formation_authority.tres global 行覆盖）
var _authority_params: Dictionary = {}
## 扫描节拍累积器（L2 基础节拍，缺省关闭时不累积）
var _authority_scan_timer: float = 0.0
## 权威值时钟（秒；仅在开关开启时按扫描节拍累积——冷却 / 评估间隔 / 错峰相位共用，
## 与帧率无关，单测可精确推进）
var _authority_clock: float = 0.0
## 单单位下一次评估时刻：iid -> float
var _authority_next_eval: Dictionary = {}
...
func set_authority_params(params: Dictionary) -> void:
	for k in params.keys():
		if _authority_params.has(k):
...
func _tick_authority_switch(delta: float) -> void:
	...
	if _squads.is_empty():
		return
	_authority_scan_timer += delta
	var beat: float = maxf(float(_authority_params.get("authority_scan_interval", 0.5)), 0.05)
	if _authority_scan_timer < beat:
		return
