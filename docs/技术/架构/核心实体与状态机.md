...
**派生属性**：
- `max_hp` = 由 `def.size` × 各层 `MaterialDef.stats.hp_per_cell` 求和
- `work_required` = 由 `def` + `MaterialDef.stats.work_per_cell` 求和
- `worker_capacity` = 由所有 `ModuleDef.work_slots_local` 数量求和
