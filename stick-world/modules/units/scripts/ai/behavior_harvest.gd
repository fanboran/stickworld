...
##   - 工位模式（work_site_def 非空，铁匠）：建筑 WorkSlots 批次 3 到位前走
##     占位定点（ProfessionRegistry.PLACEHOLDER_WORK_SITES [提案/待定]），
##     每拍 consume 原料 → produce 产出（"矿→锭"转化链；原料不足空拍等待）。
##
## params：
...
func _locate() -> bool:
	...
	if site_def.is_empty():
		...
		_mode = Mode.WORKSITE
		var site_x: float = TownLifeAPI.get_placeholder_work_site_x(site_def)
		if is_nan(site_x):
			return false  # 工位未配置（占位表未覆盖/批次 3 前过渡）
		var ground_y: float = float(entity.get("ground_y")) if "ground_y" in entity else 810.0
		_site_pos = Vector2(site_x, ground_y + WORK_OFFSET_Y)
...
func _target_valid() -> bool:
	if _mode == Mode.WORKSITE:
		return true  # 占位定点恒有效（批次 3 换 WorkSlots 时补建筑存活校验）
...
func _update_working(delta: float) -> void:
	# 资源点中途采空/失效：重新寻位（伐木工换下一棵树）
	if _mode == Mode.RESOURCE and not _target_valid():
		if not _locate():
			finish()
		return
	if entity.has_method("ai_stop"):
