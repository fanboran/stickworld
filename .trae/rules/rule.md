...
| 游戏数据表 | `config/excel/` 目录 + `docs/技术/excel-pipeline.md` |
...
6. **设计先行**：实现任何模块前，必须先用 Read 工具读取对应的设计文档（`docs/设计/系统/<模块名>.md` 和 `docs/技术/架构/模块API.md` 中该模块的 API 段落）。如果 GDD 标记了 `[待补充]`，必须向用户确认后再编码。跳过此步直接写代码会导致 API 和数据结构与设计脱节。
