...
### 临时主页菜单 + 健全地图系统（出口/导航/默认步兵）

- **临时主页菜单**（MainMenuPanel）：启动时显示测试场景预选（村庄探索/遭遇战战场/跨图跑图/森林区域），可视化版 dev_playtest；正式版替换为主菜单
- **地图系统健全**（对照 GDD §6.7）：
...
- **测试**：新增 test_menu_navigation（主页装配/显隐/动态导航/默认步兵）；全量 21 套件通过
