...
func _build_ui() -> void:
	...
	_create_order_button(order_hbox, "前进", TacticalOrders.OrderType.ADVANCE_ALL)
	_create_order_button(order_hbox, "坚守", TacticalOrders.OrderType.HOLD_POSITION)
	_create_order_button(order_hbox, "后撤", TacticalOrders.OrderType.RETREAT)
...
func _on_order_pressed(order_type: int) -> void:
	...
	if order_type == TacticalOrders.OrderType.ADVANCE_ALL:
