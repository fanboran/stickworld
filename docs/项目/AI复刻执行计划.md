...
| P10b | — | **伤害数字稀有度系统（用户 2026-09-06 指示，Pro 级发挥）**：字号随伤害非线性分级（普通伤害小一号、伤害越高越大、封顶≈现尺寸 2 倍宽）；≥50 起按稀有度**颜色分层**（如绿→蓝→紫→橙金），数字继续增大每档特效都要有区别（火焰上色/描边升级/冲击弹跳增强等游戏惯例 Pro 自主发挥）；**合金弹头式逐位数字落下**（多位数从左到右一位一位落下/滚动，slot-counter 质感）。现状：fx_library.gd `spawn_damage_text`（普通 24/暴击 34 两档、池化、恒定屏上尺寸机制保留） | 1~1.5 | 待开工（Pro 级） |
| P10c | — | **战斗伤亡播报（用户 2026-09-06 指示）**：战斗中双方伤亡损失实时播报——**按阵营分侧**：我方阵亡在我方侧屏幕、敌方阵亡在敌方侧屏幕蹦出大字（battle_banner 的 StickHand+黑描边贴纸感语言的延伸）；**连击乘号**：连续伤亡在连击窗内累计，像格斗/音游连击一样 ×2 ×3 递增显示（短时间多杀时"×3！" progressively 醒目——字号/颜色随连击档位升）。数据源：HealthComponent `died` 信号（按 faction 分侧计数，连击窗计时）；与 P10b 伤害数字系共用 StickHand 大字渲染管线 | 1 | 待开工 |
