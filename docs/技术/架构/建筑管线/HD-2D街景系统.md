...
- **实体碰撞箱（entity_collider_drawer）**：HD-2D 图画**直立脚框**——底边钉在
  角色**视觉脚线**（remap_fx_pos 含台面抬升），宽高不压、x 用物理箱真实横向
  范围（创始人 2026-09-16 裁决"碰撞箱要和脚下线框一个位置"，推翻 09-15
  "画物理位"口径）。物理箱中心 origin+(8.5, foot_offset)、脚底 origin+142
  不动。2D 图维持原中心直绘。
...
玩法自洽。**F3 角色箱贴视觉脚线**（4.4，创始人 2026-09-16 定案"碰撞箱要和
脚下线框一个位置"，推翻 09-15"画物理位"口径）——青箱与角色视觉脚线之间的
~62px·ez 前后差是"角色 vs 碰撞箱"的真实物理关系，F3 读碰撞停位时以物理箱
canvas 位（origin+foot_offset）为准。视觉系（接地影/FX/选中框/悬浮框/选中框）
锚视觉脚线。若要物理=视觉统一（origin=脚底）是牵动 AI/交互/存档的跨模块重构，
须单独立项。
