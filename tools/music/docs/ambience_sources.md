...
- **路线**：融合。破晓鸟鸣录音（**Public domain**，Dawnchorus-uk）当"远处底" + 合成稀疏叫。
- **录音处理**：低通 3000Hz + 高架 −10dB@2.6kHz → 电平 0.16 → room 混响 25%（远）。
- **换源记录（2026-09-15）**：原垫底 `birds_reveil`（Réveil des oiseaux，CC0）被创始人
  指认"听着像别的游戏解包"，弃用；换 PD 的 `Dawnchorus-uk`（无署名义务，可商用）。
  纯合成方案曾被否（创始人：别合成，找可商用采样）。
