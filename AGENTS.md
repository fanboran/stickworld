...
- 当前活跃交接文档：
  ...
  - `docs/项目/交接/音效重制-进度与交接.md`（任务：音效替换 Terraria/SWL 提取件 + 触发时机与放置优化 + 建音效管线；**代码侧（策略引擎/收口/压限请求集/空间化/暂停处置/调用点时机）全部完成并有 10 用例 294 断言的零出声测试兜底；资产侧管线已建、33+2 件待落盘**；两条实测结论见交接档 §二——引擎**不会**随树暂停停音频；2D 距离衰减只需 `Camera2D`、且监听点是**屏幕中心**（→半径须随缩放自适应）；分支 `agent/music-overhaul`，worktree `.temp/music-overhaul`）
