...
> 大战略 × 横版实时战斗 × 工厂自动化 —— 单机缝合怪。
> **引擎 Godot 4.x / GDScript · 平台 Steam PC · 画风 平面 2D 火柴人 + 手绘感笔触**

一个火柴人世界的"帝国模拟器"：从部落时代的采集建造，到编队出征的横版实时战斗，再到战略图上的大陆征服与工厂自动化。单人开发 3 个月、700+ 次提交，按 GDD 分阶段完成第一个可玩原型（定居点建设 + 小队级编制 + 小规模战斗）。

<!-- TODO(演示物料)：① 战斗演示 gif（编队接敌 / 弓手齐射 / 命中反馈，6-8s）录好放 docs/演示/战斗演示.gif；
     ② 完整 Demo 视频（1-2 分钟）传 B 站后补链接。录好后两处取消注释即可上墙。 -->
<!-- ![战斗演示](docs/演示/战斗演示.gif) -->
<!-- > 🎬 完整演示视频（B 站）：链接待补 -->
...
**十分钟体验路线**：主菜单「新游戏」→ WASD 移动，靠近树木/岩石按住 `E` 连采 → 右下「建造」放一栋建筑并施工 → 顶栏「编制」创建编队 → 带兵向右行军抵达战场歼灭敌人 → 胜利结算后自由沙盒（战略图 Tab/M、附身微操、跨图旅行、昼夜循环）。完整说明见[游戏演示页](docs/演示/游戏演示说明.md)。
...
- **机制层**：参考游戏的机制经反编译研读后以自研代码落地（Terraria 天空公式、SWL 战斗动画语义、RWR AI 参数等——机制不受版权保护，实现全部自研）
- **资产层**：音效暂用 Terraria / Stick War: Legacy 提取件 30 余个作占位，逐件登记于[素材替换清单](docs/项目/素材替换清单.md)，**正式版前全量替换**为自研合成或 CC0 素材；贴图与 BGM 为程序化自研，无外部依赖
