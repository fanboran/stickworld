...
**职责**：输入分发、附身操控。框选/编队/战术指令实现在 combat 模块（`scripts/command/`，经装配注入协同）。
...
| `DevSceneEscape` | `modules/debug_gui/scripts/dev_scene_escape.gd` | 开发场景 ESC 确认后回主页兜底（仅 debug 构建生效） | ✅ 活跃 |
