...
signal price_changed(resource_id, old_price, new_price, region_id)
# 商队到货：运输系统 → 资源系统、UI
signal trade_completed(from_region, to_region, resource_id, quantity)
# CPI 超警戒线：资源系统 → UI
signal inflation_warning(rate)
...
# 消耗沥青召唤：组织系统 → 资源系统
signal unit_summoned(unit_id, asphalt_cost)
# 晋升/调岗：组织系统 → UI
signal unit_promoted(unit_id, old_role, new_role)
# 指挥官阵亡：战斗系统 → 组织系统
signal commander_died(org_id, commander_id)
...
# 被攻击：战斗系统 → 建设系统
signal building_damaged(building_id, damage_amount)
# 升级：建设系统 → UI
signal building_upgraded(building_id, old_tier, new_tier)
...
signal tech_stalled(tech_id, reason)
...
# 进入僵持：战斗系统 → UI
signal battle_stalemate(battle_id, duration)
# 补给被切断：战斗系统 → 运输系统、组织系统
signal supply_line_cut(org_id, supply_id)
# 关键战术事件：战斗系统 → UI（可选显示）
signal tactical_event(battle_id, event_type, data)
signal territory_gained(tile_id: String)
signal territory_lost(tile_id: String)
# 文化同化完成：扩张系统 → UI
signal culture_assimilated(region_id, from_culture, to_culture)
# 包围网形成：扩张系统 → UI、战斗系统
signal coalition_formed(members)
# 条约签订：扩张系统 → UI
signal treaty_signed(type, parties, terms)

# ─────────────────────────────── 组织 ─────────────────────────────────────
# 创建新组织：组织系统 → UI
signal org_created(org_id, parent_id, tag, tier)
# 解散组织：组织系统 → UI、Project系统
signal org_disbanded(org_id)
# 重组编制：组织系统 → UI
signal org_restructured(org_id, changes)
# 效率变动：组织系统 → UI
signal org_efficiency_changed(org_id, old, new)
# AI 自主行动：组织系统 → UI（可选）
signal org_autonomy_triggered(org_id, action)

# ─────────────────────────────── 项目 ─────────────────────────────────────
# 创建项目：组织系统 → 组织系统
signal project_created(project_id, owner_org_id, type)
# 项目完成：组织系统 → 组织系统、UI
signal project_completed(project_id, result)
# 项目失败：组织系统 → 组织系统、UI
signal project_failed(project_id, reason)
# 项目分解为子项目：组织系统 → 组织系统
signal project_decomposed(parent_id, child_ids)
...
# 缩放级别变化：相机 → UI
signal ui_zoom_level_changed(new_level)
# 附身操控单位：UI → 战斗系统
signal ui_possess_unit(unit_id)
