...
static func _church(p: Painter, L: Dictionary, meta: Dictionary) -> void:
	...
	# 玫瑰窗 + 拱门 + 侧拱窗
	Comps.rose_window(p, Vector2(nave.get_center().x, wall_top + wall_h * 0.34), minf(15.0, wall_h * 0.16), p.branch("ch_rose"))
	var dw := 30.0
	...
	for k in 2:
		...
		Comps.arch_window(p, Rect2(ax - 9.0, wall_top + wall_h * 0.52, 18.0, 22.0), p.branch("ch_aw%d" % k), false)
	# 中殿屋顶
	...
	# 钟窗（拱）+ 钟
	Comps.arch_window(p, Rect2(tower.get_center().x - 10.0, tower.position.y + tow_h * 0.46, 20.0, 24.0), p.branch("ch_belfry"), false)
	Comps.bell(p, Vector2(tower.get_center().x, tower.position.y + tow_h * 0.34), 16.0, p.branch("ch_bell"))
	# 塔顶石檐 + 尖顶
	var eave := Rect2(tower.position.x - 5.0, tower.position.y - 6.0, tower.size.x + 10.0, 8.0)
	p.face(eave, "trim_white", "base")
	p.rect_stroke(eave, 2.2, {"rng": rng, "amp": 0.5})
	Comps.spire(p, eave, {"spire_h": 54.0, "mat": "slate"}, p.branch("ch_spire"))
...
static func _chapel(p: Painter, L: Dictionary, meta: Dictionary) -> void:
	...
	Comps.arch_window(p, Rect2(px_w * 0.2, body.position.y + wall_h * 0.42, 18.0, 24.0), p.branch("cp_w1"), false)
	Comps.arch_window(p, Rect2(px_w * 0.66, body.position.y + wall_h * 0.42, 18.0, 24.0), p.branch("cp_w2"), false)
	Comps.door(p, Rect2(px_w * 0.5 - 14.0, baseline - wall_h * 0.56, 28.0, wall_h * 0.56), "wood_dark", p.branch("cp_door"), false, true)
	var roof := {"rise": 34.0, "overhang": 10.0, "mat": "slate"}
	Comps.gable_roof(p, body, roof, p.branch("cp_roof"), true)
	# 山墙顶尖塔
	var apex := Vector2(body.get_center().x, body.position.y - 34.0)
	Comps.spire(p, Rect2(apex.x - 9.0, apex.y, 18.0, 6.0), {"spire_h": 30.0, "mat": "slate"}, p.branch("cp_spire"))
	Common.ground_shadow(p, L, 1.0)
...
static func _tower(p: Painter, L: Dictionary, meta: Dictionary) -> void:
	...
	for i in 3:
		...
		Comps.arch_window(p, Rect2(body.get_center().x - 6.0, y, 12.0, 18.0), p.branch("tw_w%d" % i), false)
	Comps.door(p, Rect2(body.get_center().x - 13.0, baseline - wall_h * 0.26, 26.0, wall_h * 0.26), "wood_dark", p.branch("tw_door"), false, true)
	Comps.battlement(p, body.position.x - 4.0, body.end.x + 4.0, wall_top, 12.0, "stone_dark", p.branch("tw_batt"))
	# 塔身收分线
...
static func _gatehouse(p: Painter, L: Dictionary, meta: Dictionary) -> void:
	...
	# 门洞上方小窗
	Comps.arch_window(p, Rect2(px_w * 0.5 - 8.0, wall_top + wall_h * 0.14, 16.0, 20.0), p.branch("g_win"), false)
	# 两侧塔
	for k in 2:
		var tx := 2.0 if k == 0 else px_w - tow_w - 2.0
		var tower := Rect2(tx, wall_top - 12.0, tow_w, wall_h + 12.0)
		Comps.wall_plane(p, tower, "stone", "base", 2.8)
		Comps.arch_window(p, Rect2(tower.get_center().x - 7.0, wall_top + wall_h * 0.3, 14.0, 18.0), p.branch("g_tw%d" % k), false)
		Comps.battlement(p, tower.position.x - 3.0, tower.end.x + 3.0, wall_top - 12.0, 12.0, "stone", p.branch("g_batt%d" % k))
	# 中间垛口
	Comps.battlement(p, tow_w + 6.0, px_w - tow_w - 6.0, wall_top, 12.0, "stone", p.branch("g_battm"))
	Common.ground_shadow(p, L, 1.0)
...
static func _wall_seg(p: Painter, L: Dictionary, meta: Dictionary) -> void:
	...
	Comps.battlement(p, body.position.x, body.end.x, wall_top, 11.0, "stone", p.branch("w_batt"))
