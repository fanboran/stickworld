...
- **启动/流程**：主菜单接入（继续/新游戏/读档/设置/退出）、载入屏（双进度条两级进度：上条=总阶段、下条=阶段内细分）、**战略图装配步骤表分帧**、世界加载覆盖层（真实加载有指示、无死灰屏）
...
- **HUD**：部件定位归 `hud_zone_layout.gd` zone 注册表（六角部件 Minimap/ZoomBar/资源条/任务卡/时钟组/通知流全部入区；缩放条=top_center 堆叠区，Minimap 正下方）；顶栏距边 12px、小地图 1.5 倍、缩放条条本体对齐+100%刻度+文字在条右侧、时钟缝隙对称

### 下一步（接手 AI 的待办）

1. **做做样子项清理收尾**：通知 feed 堆叠 / 游戏内快捷栏 / 设置项"存了不生效"三项已完成（`notification_feed.gd` 堆叠 feed、Hotbar 经 InventoryService 接真实指令、设置项真实生效 `test_settings_apply`）；余**小地图样式统一**（ZoomBar 已走 SketchHSlider 皮肤，Minimap 色板仍独立硬编码）
