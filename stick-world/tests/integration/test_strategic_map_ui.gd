...
func _test_l1_title_legend() -> void:
	...
	# 图例（B4 默认 TERRAIN 地形模式）：地物水系 + B2 群系色 + F5 道路分级
	# （共 10：海洋/湖泊/平原/森林/荒漠/冰原/水源带/火山/土路/官道）
	...
	_runner.assert_true(_l1_legend._entries_box.get_child_count() == 10,
		"地形模式 10 条目（实测 %d）" % _l1_legend._entries_box.get_child_count())
	# 切政治模式：8 城邦政权色 + 道路分级条目，政权色块与地图填充同色源（get_state_color）
	...
	_runner.assert_true(_l1_legend._entries_box.get_child_count() == 10,
		"8 城邦 + 2 道路条目（实测 %d）" % _l1_legend._entries_box.get_child_count())
