...
- 当前活跃交接文档：
  ...
  - `docs/项目/交接/加载屏双进度条与分帧装配-进度与交接.md`（任务：治加载卡顿 + 两级进度条（上=总阶段/下=阶段内细分）+ dev 探针静音后置底；**批次 1 + 批次 2 第一项完成**——启动预热器 + 渲染后端 Windows 改 vulkan（D3D12 树木首绘管线编译冻结实测 127s→3.8s），进世界实测 ~152s→~30s，待创始人观感验收（含 vulkan 观感）；剩余候选与完整量测数据见文档 §二/§三，⚠ 启动自动写存档 slot 0 见 §四，分支 `agent/loading-ux`，worktree `.temp/loading-ux`）
