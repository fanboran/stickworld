...
static func _barn(p: Painter, L: Dictionary, meta: Dictionary) -> void:
	...
	# 山墙大顶 + 通风窗
	Comps.gable_roof(p, body, {"rise": 58.0, "overhang": 12.0, "mat": "roof_wood"}, p.branch("b_roof"), true)
	Comps.hay_pile(p, Vector2(body.end.x - 26.0, baseline - 1.0), 34.0, p.branch("b_hay"))
...
static func _stable(p: Painter, L: Dictionary, meta: Dictionary) -> void:
	...
	var out := 24.0
	p.quad_face([Vector2(-out, eave_y), Vector2(px_w + out, eave_y), Vector2(px_w - 2.0, wall_top - 30.0), Vector2(2.0, wall_top - 30.0)], "thatch_dry", "base")
	p.stroke(PackedVector2Array([Vector2(-out, eave_y), Vector2(px_w + out, eave_y)]), 3.0, {"rng": rng, "amp": 0.6})
	p.stroke(PackedVector2Array([Vector2(2.0, wall_top - 30.0), Vector2(px_w - 2.0, wall_top - 30.0)]), 2.0, {"rng": rng, "amp": 0.5})
	Comps.eave_shade(p, Rect2(Vector2(-out, eave_y), Vector2(px_w + out * 2.0, 9.0)), 0.22)
	# 门 + 干草 + 栅栏
	Comps.door(p, Rect2(px_w * 0.72, baseline - wall_h * 0.56, 26.0, wall_h * 0.56), "wood_dark", p.branch("st_door"))
	Comps.hay_pile(p, Vector2(body.get_center().x * 0.55 + open_w * 0.5, baseline - 1.0), 30.0, p.branch("st_hay"))
	Common.ground_shadow(p, L, 1.0)
	Common.body_barrier(meta, body)
...
static func _windmill(p: Painter, L: Dictionary, meta: Dictionary) -> void:
	...
	# 门
	Comps.door(p, Rect2(px_w * 0.5 - 13.0, baseline - wall_h * 0.3, 26.0, wall_h * 0.3), "wood_dark", p.branch("wm_door"), false, true)
	# 锥顶
	var cap := PackedVector2Array([tl, tr, Vector2(px_w * 0.5, wall_top - 30.0)])
	p.poly(cap, p.mat_color("roof_wood", "base"))
	p.hatch(cap, 8.0, -50.0, 1.1, 0.2)
	p.stroke(PackedVector2Array([tl, tr, Vector2(px_w * 0.5, wall_top - 30.0), tl]), 2.6, {"rng": rng, "amp": 0.5})
	# 风车叶（hub 在塔身上部，四叶十字）
	var hub := Vector2(px_w * 0.5, wall_top + wall_h * 0.12)
	Comps.windmill_blades(p, hub, minf(42.0, px_w * 0.44), p.branch("wm_blades"))
	Common.ground_shadow(p, L, 1.0)
