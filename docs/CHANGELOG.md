...
### Tab L1 出生地块更正为 region_013 3 号 + 全部周边陆地 + 居中 + 整图适配（2026-08）
...
- **周围全部陆地显示**：context 以出生 L1 质心为中心扩到 700×700，圈入 **18 个周边老 L1 块**（灰色邻居）全部陆地 + 海洋 + 湖泊；生成脚本新增 `--start-l1 69 --half 350`（context 半宽）
- **出生 L1 居中**：context 中心 = 出生 L1 质心（`focus_center`），打开即居中（此前 context 偏到出生块 + 单一邻居，出生块不居中）
- **默认缩放 = 整图适配（100%）**：`DEFAULT_ZOOM_MULT` 1.75 → 1.0，打开即见全部周边陆地（HUD 记为 100%）
- **F3 城市编号屏幕恒定字号**：`LABEL_SCREEN_SIZE=22`（÷缩放 clamp），L1 高缩放下不再"雷霆大字"
