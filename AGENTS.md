...
- 当前活跃交接文档：
  ...
  - `docs/项目/交接/火柴人视觉修复-进度与交接.md`（任务：描边融合+缩放抗锯齿；**批次 1-4h 完成；批次 4i 逆向重建启动（创始人拍板解包真实数据）：真相源已从 APK 重提、覆盖率方案已落档（`docs/设计/系统/火柴人逆向重建与覆盖率方案.md` 接手必读）、旧提取器已删、IK 死重已清，批次 A~D（新导入器→渲染重标→数值验收→视觉验收）待做**；worktree `.temp/stickman-visual` 保留）
