...
static func extra(p: Painter, L: Dictionary, meta: Dictionary, name: String, rng: RandomNumberGenerator) -> void:
	...
	match name:
		...
		"chimney", "chimney_big":
			...
			var cw := 34.0 if big else 24.0
			var cx := body.position.x + px_w * (0.26 if not big else 0.78)
			var ctop := wall_top - rise * 0.72 - (48.0 if big else 36.0)
			Comps.chimney(p, Rect2(cx - cw * 0.5, ctop, cw, wall_top - ctop + rise * 0.3), ctop, mat, rng)
		"sign":
			var sx := body.position.x + px_w * 0.22
			Comps.sign_board(p, Vector2(sx, wall_top + 28.0), 1, rng)
		"lantern":
			Comps.lantern(p, Vector2(body.end.x - px_w * 0.18, wall_top + 8.0), rng)
		"flag":
			Comps.flag(p, Vector2(body.get_center().x, wall_top - rise), 46.0, "cloth_red", rng)
		"bell":
			var bc := Vector2(body.get_center().x, wall_top - rise * 0.42)
			p.stroke(PackedVector2Array([Vector2(bc.x - 16.0, wall_top - rise * 0.62), bc + Vector2(-16.0, 11.0)]), 2.2, {"rng": rng, "amp": 0.3, "taper": false})
			p.stroke(PackedVector2Array([Vector2(bc.x + 16.0, wall_top - rise * 0.62), bc + Vector2(16.0, 11.0)]), 2.2, {"rng": rng, "amp": 0.3, "taper": false})
			p.stroke(PackedVector2Array([Vector2(bc.x - 19.0, wall_top - rise * 0.62), Vector2(bc.x + 19.0, wall_top - rise * 0.66)]), 2.2, {"rng": rng, "amp": 0.3, "taper": false})
			Comps.bell(p, bc, 30.0, rng)
		"awning":
			# 首层窗上条纹遮阳棚（商铺）
			var aw := px_w * 0.72
			...
			var top_l := Vector2(ac.x - aw * 0.44, ay - 28.0)
			var top_r := Vector2(ac.x + aw * 0.44, ay - 28.0)
		...
		"barrel":
			Comps.barrel(p, Vector2(body.position.x + 24.0, baseline - 2.0), 42.0, rng)
			Comps.crate(p, Rect2(body.end.x - 48.0, baseline - 33.0, 33.0, 30.0), rng)
		"hay":
			Comps.hay_pile(p, Vector2(body.end.x - 38.0, baseline - 1.0), 54.0, rng)
		"stairs":
			# 外部楼梯（贴左端正面爬升，12 级 + 侧梁 + 扶手）
			var steps := 12
			var sx0 := body.position.x + 6.0
			var step_w := 30.0
			var step_h := (float(L["wall_h"]) * 0.42) / float(steps)
			for i in steps:
				var sy := baseline - step_h * float(i + 1)
				var sr := Rect2(sx0 + step_w * float(i), sy, step_w + 4.0, step_h)
				p.face(sr, "stone_dark", "base")
				p.stroke(PackedVector2Array([sr.position, Vector2(sr.end.x, sr.position.y)]), 2.0, {"rng": rng, "amp": 0.4})
			# 侧梁与扶手
			var top_y := baseline - step_h * float(steps)
			p.stroke(PackedVector2Array([Vector2(sx0, baseline), Vector2(sx0 + step_w * float(steps), top_y)]), 3.4, {"rng": rng, "amp": 0.4})
			p.stroke(PackedVector2Array([Vector2(sx0, baseline - 34.0), Vector2(sx0 + step_w * float(steps), top_y - 34.0)]), 2.6, {"rng": rng, "amp": 0.4})
		"fence":
			Comps.fence(p, body.position.x + 4.0, body.end.x - 4.0, baseline - 1.0, 28.0, rng)
		_:
			pass
