...
- 2026-09-11（续）：**刀③落地——修改器栈默认禁用 + 骨架内部处理永久
  关闭**。验证发现栈的解算 override 每帧被动画 track 覆盖、实为死重
  （姿态实测无差），不需要「代码解算 IK」——直接禁栈即终局，_solve_hz
  限频机制一并删除；`STICK_RIG_IK=1` 开回旧行为。机制、验收与**量测污染
  警示（本机分钟级负载波动 ±30%，今日 headless 绝对数字全作废，量测
  纪律见 §十二）** 落档 §十二。分桶框架（刀①）在刀③后跳过帧四块全省，
  收益幅度待安静环境复测。渲染侧剩余方向（§十一 排序）：②非 rig 绘制
  剔除/合批（血条/阴影/武器 ~700-900 draws）④批渲染更新频率与 LOD
  档位联动；以及立项「观感验收」：96/48 同 zoom 同战斗阶段 4× 放大
  A/B（创始人）。
