...
function syncOverlays(){const s=dockSpace();
 mini.style.bottom=(s?s+12:32)+"px";   // 32=状态条(24)+8：小地图不再压 cfoot 文字
...
const GUT=70,AXH=18;  // 左标签列宽 / 顶部时间轴高（Frappe Gantt 惯例：标签列独立于绘图区、轴在顶部）
function drawGantt(){const c=gantt;if(!c)return;
 ...
 for(let x=GUT;x<W;x+=40){gtx.moveTo(x,0);gtx.lineTo(x,H);}for(let y=0;y<H;y+=40){gtx.moveTo(0,y);gtx.lineTo(W,y);}gtx.stroke();
 ...
 const padL=GUT,padT=AXH+4,padB=16,rowH=15;
 ...
 // x 缩放：滚轮/拖拽平移（存 ganttView）；绘图区从 GUT 起——标签列不再被条压住
 if(!ganttView)ganttView={t0:0,t1:tMax};
 const span=ganttView.t1-ganttView.t0||1;
 const plotW=W-padL-8;
 const X=t=>padL+(t-ganttView.t0)/span*plotW;
 // 时间轴（顶部表头，Frappe 式）：刻度标签在轴带内，右侧标单位——曾画在底部与图例重叠
 gtx.fillStyle="#0b111c";gtx.fillRect(0,0,W,AXH);
 ...
 for(let t=Math.ceil(ganttView.t0/stepT)*stepT;t<=ganttView.t1;t+=stepT){
  const x=X(t);gtx.strokeStyle="#101927";gtx.beginPath();gtx.moveTo(x,padT-2);gtx.lineTo(x,H-padB);gtx.stroke();
  gtx.fillText("T"+String(t).padStart(3,"0"),x+3,AXH-5);}
 gtx.textAlign="right";gtx.fillStyle="#55677f";gtx.font="9px system-ui";
 gtx.fillText("时间 →（SIM=轮次 · 真实=分钟）",W-6,AXH-5);gtx.textAlign="left";
 // 标签列底 + 泳道名称（独立列，条形不再侵入）
 gtx.fillStyle="#0b111c";gtx.fillRect(0,AXH,GUT,H-AXH);
 bandNames.forEach((l,i)=>{const y=padT+i*bandH;
  if(i%2===1){gtx.fillStyle="#0b111c";gtx.fillRect(GUT,y,W-GUT,bandH);}   // 隔带条纹只涂绘图区（此前误用 band 总数 parity）
  const li=lanes.indexOf(l),lc=LANE_COL[(li<0?0:li)%LANE_COL.length];
  gtx.fillStyle=lc+"22";gtx.fillRect(0,y,GUT-4,bandH);
  if(ganttHoverBand===l){gtx.fillStyle=lc+"14";gtx.fillRect(GUT,y,W-GUT,bandH);}
  gtx.fillStyle=lc;gtx.font="600 8.5px "+MONO;
  gtx.save();gtx.translate(8,y+bandH/2+3);
  let lab=l;while(lab&&gtx.measureText(lab).width>GUT-20)lab=lab.slice(0,-1);  // 标签列内自适应截断
  gtx.fillText(lab,0,0);gtx.restore();});
 // 任务条：同带子行贪心（3px 圆角，Frappe 惯例）
 const rows={};
 ...
 sorted.forEach(j=>{const b=bandOf[j.lane]!=null?bandOf[j.lane]:bandNames.length-1;
  ...
  const bw=Math.max(3,x1-x0),bh=rowH-6;
  gtx.fillStyle=j.done?lc+"55":ac+"33";
  gtx.beginPath();gtx.moveTo(x0+3,y);gtx.arcTo(x0+bw,y,x0+bw,y+bh,3);gtx.arcTo(x0+bw,y+bh,x0,y+bh,3);
  gtx.arcTo(x0,y+bh,x0,y,3);gtx.arcTo(x0,y,x0+bw,y,3);gtx.closePath();gtx.fill();
  gtx.strokeStyle=j.done?lc:ac;gtx.lineWidth=1;gtx.stroke();
  if(!j.done){ // 运行中=右缘琥珀光标条
   gtx.fillStyle="#fde047";gtx.fillRect(x1-2,y,2,bh);}
  if(x1-x0>34){gtx.fillStyle="#c9d4e0";gtx.fillText(j.id,x0+4,y+8);}
 ...
 if(di)di.textContent="■完成 ▌运行中 · 色=泳道 描边=agent · 滚轮缩放/拖拽平移/点条跳图";
  ...
  if(sw&&!sw._built){sw._built=true;   // agent 筛选=下拉（21 个 agent 按钮曾横向溢出 dockBar）
   const agents=[];evs.forEach(e=>{if(e.agent&&agents.indexOf(e.agent)<0)agents.push(e.agent);});
   sw.innerHTML="<span style='color:var(--dim2);font:600 10px "+MONO+";padding:0 2px'>agent</span>"
    +"<select id='agentSel' onchange='setAgentFilter(this.value||null)' title='按认领 agent 过滤运行图'>"
    +"<option value=''>全部</option>"
    +agents.map(a2=>"<option value='"+a2+"'"+(agentFilter===a2?" selected":"")+">"+a2+"</option>").join("")
    +"</select>";}}
function setAgentFilter(a){agentFilter=a||null;
 const s=document.getElementById("agentSel");
 if(s&&s.value!==(agentFilter||""))s.value=agentFilter||"";
 drawGantt();}
...
gantt.addEventListener("wheel",ev=>{ev.preventDefault();
 ...
 const anchor=ganttView.t0+(ev.offsetX-GUT)/(gantt.clientWidth-GUT-8)*(ganttView.t1-ganttView.t0);
...
gantt.addEventListener("pointermove",ev=>{
 ...
 const span=gDrag.t1-gDrag.t0,d=(ev.clientX-gDrag.x)/(gantt.clientWidth-GUT-8)*span;
...
function hitGantt(x,y){ // 命中检测：{type:"task",id} 或 {type:"band",lane}（空带/标签列区域=聚拢该泳道）
 if(!ganttLayout||!ganttView||!ganttJobs)return null;   // 底栏未画过=无几何可命中
 const{bandNames,bandOf,bandH,padT}=ganttLayout;
 const span=ganttView.t1-ganttView.t0||1,W=gantt.clientWidth-GUT-8;
 const hit=Object.keys(ganttJobs).find(id=>{const j=ganttJobs[id];
  ...
  const x0=GUT+(j.t0-ganttView.t0)/span*W,x1=j.done?GUT+(j.t1+1-ganttView.t0)/span*W:W+GUT+8;
  return y>=y0&&y<=y0+bandH&&x>=x0-4&&x<=x1+4;});
 if(hit)return{type:"task",id:hit};
 const bi=Math.floor((y-padT)/bandH);
 return (bi>=0&&bi<bandNames.length&&y>=padT)?{type:"band",lane:bandNames[bi]}:null;}
