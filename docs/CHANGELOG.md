...
- **夜版卡（月光烘进 Blender）**：三套烘卡脚本（blender_proto/bake_props/bake_nature）每卡多烘一张 `<卡>_night.png`——亮冷蓝月亮方向光（正面高角度斜打）+ 深蓝夜环境，窗/火/水晶叠**半透明发光**（copy 原材质 + Add Shader 并联 Emission，原材质透出；整块换发光片的首版把窗吹成白块、被创始人改口径）；建筑 50 + 道具 48 + 自然物 15 全量入库 `tex/`（113 张）。另修两坑：月亮首版从背面打光致正立面全黑（定稿 (55°,-75°)）；道具库裸 `glass` 是井口/水面占位材质，进发光名单会把井口烘成白圈（创始人指认），道具发光名单摘除裸 `glass`
