extends SceneTree
## 直接 CPU 合成 smithy_preview.tscn 为 PNG（避开 headless SubViewport 截图问题）
## 输出：modules/building_gen/reference/smithy_preview_composite.png

const SCENE_PATH := "res://modules/building_gen/scenes/smithy_preview.tscn"
const OUT_PATH := "res://modules/building_gen/reference/smithy_preview_composite.png"
...
func _render_to_image(root: Node2D, scale: float) -> Image:
	...
	# 3) 按原场景层级顺序绘制（先递归后绘制，children 在后覆盖在前）
...
func _draw_node_recursive(node: Node, canvas: Image, canvas_pos: Vector2i,
	...
	if node is Node2D:
		...
		var final_scale := parent_scale * n2d.scale.x  # 统一缩放（假设等比）
...
func _draw_polygon(canvas: Image, p: Polygon2D, canvas_pos: Vector2i, scale: float, gpos: Vector2, grot: float) -> void:
	...
	# 是否为茅草贴图：编辑器里 ShaderMaterial 会强制内部不透明并羽化边缘
	var is_thatch := tex is ImageTexture and tex.has_meta("thatch_inner_size")

	# 计算 polygon 在目标画布上的 AABB
	var min_x := 1e9
	var max_x := -1e9
	var min_y := 1e9
	var max_y := -1e9
	...
	for y in range(iy0, iy1 + 1):
		for x in range(ix0, ix1 + 1):
			...
			var sx := clampf(uv.x, 0.0, float(src_w - 1))
			var sy := clampf(uv.y, 0.0, float(src_h - 1))
			var c := _sample_bilinear(img, sx, sy)
			if c.a <= 0.0:
				continue
			# 茅草贴图在编辑器中由 Shader 强制内部不透明，CPU 合成时做同等处理
			if is_thatch:
				c.a = 1.0
...
func _sample_bilinear(img: Image, sx: float, sy: float) -> Color:
	var x0 := int(sx)
	var y0 := int(sy)
