...
- **地形分行线（terrain_grid）**：HD-2D 图**不画**——32px 分行语义属于 2D 图
  ...
  （创始人 2026-09-15）。
- **实体碰撞箱（entity_collider_drawer）**：HD-2D 图画在**物理碰撞位**——箱
  中心经 remap_fx_pos（含台面抬升）压进 3D 投影域，宽高不压、x 用物理箱真实
  横向范围。**F3=碰撞真相视图，所有箱子都画在真实碰撞位、同一投影域**：角色
  被蓝/紫带挡停时青箱恰好压在对方带上，"碰上即停"逐像素可读（创始人
  2026-09-15：绘制的箱子移动到碰撞箱位置，不迁就视觉脚线——此前画在视觉
  脚线的版本会"穿过蓝带停不住"，即物理箱在 origin+142 前方、显示箱在 origin
  的脱钩）。物理脚箱中心 origin+(8.5, foot_offset)、脚底 origin+142 不动。
  2D 图维持原中心直绘。
...
- **鼠标世界坐标**：y 走 4.2 逆变换；标签钉鼠标屏幕位（世界 y 已是地面带坐标，
  不能再经 2D 投影回屏幕）。
- **选中/悬浮四角框（SelectionSystem）**：HD-2D 图锚**视觉脚线**（origin 经
  remap_fx_pos）——origin 按 2D 直绘会比角色高 (1−k)×纵深距离（y=1000 时
  ≈124px@0.75，创始人 2026-09-15"悬浮方框比角色高很多"）；possessed 主控框
  走 3D 脚下框（wants_3d_bracket 门控），位置天然正确。
...
玩法自洽。**F3 角色箱画在物理碰撞位**（4.4，创始人定案：F3=碰撞真相，箱子
不迁就视觉脚线）——角色被挡停时青箱恰好压在对方带上；角色视觉脚线与青箱之间
的 ~62px·ez 前后差是"角色 vs 碰撞箱"的真实关系，不是显示错误。视觉系（接地
影/FX/选中框）锚视觉脚线。若要物理=视觉统一（origin=脚底）是牵动 AI/交互/
存档的跨模块重构，须单独立项。
