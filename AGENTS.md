...
- 当前活跃交接文档：
  ...
  - `docs/项目/交接/游戏AI集大成-进度与交接.md`（任务：业界 AI 机制复刻与分层集成——CoH/RWR/SWL 三家登记，批次 A1~A9，设计文档 `docs/设计/系统/12-游戏AI系统.md`；**A1、A9 完成，A2 实施中，A3~A6 排队；组织批次 3 基建随 `agent/organization-deepening` 收线后合并**，分支 `agent/game-ai`，worktree `.temp/game-ai`；main 上此交接档为登记快照，最新版以分支为准）
