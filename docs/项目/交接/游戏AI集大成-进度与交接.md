...
- **当前阶段**：**A 系列 + 观测接线批 + WorldBox WB1/WB2/WB6 + AI-GAPS + AUTHORITY-SWITCH + UI-W2（班组卡 / 组织面板与上报流）全部收官并合入 main**。创始人裁决**不再实机验收**，机制开闸由 AI 线按设计文档 §七 清单**逐层开闸并留档**（每开一批跑零回归 + 可观测指标写成报告，创始人抽空看报告）。开闸进度：**GK-1 已开**（bow 点射 3 发 / 夜间犹豫 1.8 / 溃逃横向游走），GK-2~GK-5 待开（GK-5 前置=组织侧 `default_behavior` 配置写入方，当前无写入方 → 效用打分宿主开了也没有候选）。界面线下一批 UI-W3 指挥链可视化（需先补在途命令注册表）。
...
**开闸会撞既有断言**：各批单测里有大量"开关关 = 零回归"的用例（如 A9 R3 的"门禁关"、A6 的"压制未启用"）。开闸后这些用例会失败——这不是回归，而是**断言锁的是旧默认值**。处理规矩：逐条把"锁默认关"改成"锁新默认"或改成"显式注入覆盖后测两态"，并在报告里逐条说明改了哪条断言、为什么不是掩盖回归。**禁止**为了让测试变绿而放宽断言强度。

**开闸值要有依据**：机制开着不等于数值对。每个开闸值必须写出来源（RWR/CoH 逆向真值直译、按节拍比换算、或明确的保守初值）并标注"待校准"——没有实机反馈时宁可取保守档。兵种相关机制优先落**兵种覆盖档行**（RWR per-class .ai 覆盖同构），不无差别写进 baseline 行（例：点射只开在远程兵种，包抄只开在近战兵种）。
...
| AI-GAPS（A6/WB2 遗留） | ✅ 完成 | 见 git log | **箭矢近失压制 + 决策时钟随档往返**：①近失压制——箭矢非命中终态（插地）对落点半径内敌方单位施加压制，只压制不伤血、不走 DamagePipeline；命中仍走既有受击门槛路径（两条不重复）；射手阵营按发射瞬间快照过滤（射手随后阵亡也不误压友军）；档案键 `suppression_near_miss_enabled`(默认关)/`suppression_near_miss_radius`(80px，须 > 命中半径 34)，受 `suppression_enabled` 总门约束；口径取舍=只认落点半径，不做逐帧擦身（逐帧成本高且会因箭飞越前排造成群体误压）；②status_effects 抽出共用豁免 `_can_suppress()` + 通用入口 `apply_suppression()`；③AI 时钟族序列化——`ai_controller.export/import_timing_state` 存**相对剩余量**，save_handler 写 `extra_data.ai_timing`、读档在 spawn 之后回填（不重掷错峰假偏移，否则错峰变双重随机）；老档无字段安全回退、字段损坏逐项跳过；④test_suppression 13→18 用例、新套件 test_ai_timing_save 6 用例 31 断言 |
| AUTHORITY-SWITCH（A9 R4 补全） | ✅ 完成 | 821354db | **士兵自主跳槽行为实装**（创始人裁决）：①formation_system 新增权威值跳槽段，挂既有 L2 节拍宿主 `_process` 末尾（与队伍目标决策/相位计划同宿主同 0.5s 基础节拍），复用既有 `get_squad_authority`/`should_switch_squad` 与 `authority_margin` 滞回；②新档 `config/ai/formation_authority.tres`（`authority_switch_enabled` 默认关 + 扫描/评估间隔、跳槽冷却 20s、候选半径 800px、玩家班吸引 0.2 与黏性 0.5、班长放人阈值 0.3（后三项 RWR 真值直译）、单拍限流 1）；③六道守卫：玩家附身不搬动 / 班长与在册指挥官不外流 / 溃逃找掩体不换班 / 真实压制不换班 / 接战中不换班 / 玩家号令保护期内不换班（复用 TeamAi 唯一真相源，新公开 `is_manual_order_guarded` 转发口）；④防振荡=margin 滞回 + 单位 2s 评估间隔 + 确定性错峰相位（按登记序而非 instance_id，跨运行可复现）+ 单拍单来源班限流 1；⑤只评估战斗班（不把劳工/建造/运输队卷进人事市场）；⑥新套件 test_authority_switch 14 用例 75 断言——每道守卫都配**对照者断言**（对照者必须换班，以证明该场景本来会换人） |
| UI-W2-A 班组卡 | ✅ 完成 | 163f2596 | **L1 班组卡落位 ContextPanel/SquadInspector 槽**：①新档 squad_card + squad_member_row（场景=布局唯一真相源）——头部班名+状态徽标、号令栏（含上级组织号令上溯）、相位徽标直译、成员行（角色角标/士气微型条/被压制·溃逃·治疗状态）、班长栏（威望星级）、两条既有操作+指挥链入口占位；成员行上限 8 超出压成"还有 N 人未列出"；②取数全 duck 探测逐项降级，相位/角色/号令/编制/选择信号驱动，仅士气等缓变值走 0.25s 低频节拍；③**修既存缺陷**：ContextPanel 原 `anchors_preset=9`(LEFT_WIDE)+`offset_left=-240` 实为**锚在屏幕外**（空置故从未暴露，班组卡作为首个消费者才暴露）→ 改 RIGHT_WIDE 并在场景声明槽；补 `mouse_filter=IGNORE`（空槽不吃世界框选）与 `clear_context` 只清运行时动态内容（否则切模式时场景声明的槽被 queue_free）；④新集成套件 test_squad_card 10 用例（含槽存活/空态不塌/清空选择零残留）；⑤遗留：OrgPanel「选中 L1 组织」触发源未接（其选中态无信号，已留 `show_squad()` 一行入口）、组织态 State 无 ACTIVE 迁移实现故"组建中/招兵位"仅在空班成立 |
| UI-W2-B OrgPanel+上报流 | ✅ 完成 | 58404192 | **组织面板状态化 + 上报/补位叙事**：①OrgPanel 树节点加状态母题图标与徽标（状态/辖N人/士气/群龙无首），超宽逐项降级到悬停 tooltip（含补位候选序前 3）；详情区新增组织概览卡（士气手绘条+空缺警示+候选序）；「群龙无首」= tier>1 且指挥官空缺，L1 空架不算（不制造假警报）；②士气聚合用「当前地图在场实体表」建索引而非全局 `instance_from_id` 反查——后者对脏档/测试桩的非法 id 会触发 ObjectDB 槽位越界报错刷日志；③新档 org_report_narrator（常驻 GameRoot）消费 `report_filed` 三型上报 + `commander_assigned` 做补位叙事（filled 真/假两分支，payload 为唯一权威；**只有 filled=true 才说"自动接任"**）；通知走既有 `EventBus.ui_notification` → NotificationFeed，不新造组件；④门控不绕过——叙事器只订阅 `report_filed`，不做二次判定（该信号本就只在组织侧 `evaluate_report_gate` 之后才发，重复判定会与组织侧规则分叉）；⑤新集成套件 test_org_report_narrator 6 用例 / test_org_panel_badges 5 用例 |
| 集成层漏测自检（infra） | ✅ 完成 | 58404192 | **run_all.sh 清单自检**：unit 层的"盘上有、清单无=永不运行"盲区在 integration/smoke 层同样存在（run_all 有独立清单：本批实测原 41 条清单 vs 盘上 44 个 `.tscn`，差集正是两条新套件 + 一个**既存孤儿**）。补 `manifest_check`（盘上/清单比对 + `UNREGISTERED_ALLOWLIST` 显式豁免理由 + 未登记即 exit 1），负向探针实测能拦下未登记文件、正向放行；顺带查出既存孤儿 `test_ui_overlap.tscn`——从未登记从未运行、非 TestRunner 规范（自打印 FAIL 且退出码恒 0）、断言口径是 zone 制之前的老布局 → 入豁免名单并写明理由，待处置（重写或下沉 tests/dev） |
| A7 | ⬜ 挂起 | — | 待经济域立项 |
| A8+ | ⬜ 挂起 | — | 待创始人裁决 |
...
| §七 开关表读法 | 「默认」列读作**生效默认**，不等于"baseline 行的值"——合并序为 代码 BASELINE ← 代码兵种覆盖档 CLASS_PROFILES ← .tres 行，故某键"baseline 里是关"不代表没生效。`flank_enabled`/`arrow_threat_block`/`missing_arrows_tolerance`/`heal_enabled` 四项早已由 CLASS_PROFILES 为对应兵种开启（曾把生效中的误标为"默认关"，已修正） |
