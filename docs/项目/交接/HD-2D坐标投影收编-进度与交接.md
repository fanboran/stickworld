...
**第五轮（创始人复验反馈，2026-09-16）**：
- **缩放条整 10 档**：滑块改显示百分比域 70%~260%、步进 10%（20 档）——旧
  user_zoom 域 0.5~2.0 换算显示 66.7%~266.7%，端点非整 10；user_zoom=显示%×
  0.75/100，默认 100% 恰在刻度上；滚轮仍走 CameraRig 步进，句柄吸附最近刻度。
- **街边摆件碰撞带下偏 ~32px**：实测摆放链路零偏差（probe_props），真因=
  碰撞带**基线前后各跨 26px**，前缘伸到基线前方，角色在基线前 26+12（脚框
  半宽）=38px 被挡（38·ez@113% 档 ≈32px，创始人读数吻合）。修=纵深带改
  **基线向街心延伸** [y_c−52, y_c] 不前伸（proto `_spawn_prop`）。实测工具
  `tests/dev/probe_props.tscn`（dump 视觉落点 z vs 碰撞带中心 z）。
- 本轮报错自检的 4 个 sqlite DLL 报错为创始人开着游戏/编辑器会话锁 DLL 的
  环境噪音（编辑器启动模拟复制失败），非代码问题。
