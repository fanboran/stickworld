...
主菜单「新游戏」→ 四阶段引导：**采集**（WASD 移动，靠近树木/岩石按 `E` 连采）→ **建造**（右下「建造」放建筑并施工）→ **编队**（顶栏「编制」创建编队，或 `Q` 框选士兵）→ **战斗**（带兵行军抵达战场，歼灭敌军）→ 胜利结算，开放自由沙盒（战略图 Tab/M、附身微操、跨图旅行、昼夜循环）。
...
**v0.5.0（当前）**：首个公开可玩 Demo——四阶段引导闭环 + 自由沙盒收口。
