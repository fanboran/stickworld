extends Node
## 集成测试：框选系统（SelectionSystem，原 test_stage_06 迁移）。
##
## 运行：
...
func _test_box_select() -> void:
	...
	# unit 0 和 4 不应被选中
	_runner.assert_true(not _selection.is_selected(_helper.units[0]), "unit 0 不应被选中")
	_runner.assert_true(not _selection.is_selected(_helper.units[4]), "unit 4 不应被选中")
...
func _test_additive_select() -> void:
	...
	_selection.clear_selection()
	# 先选 unit 0
	var rect0 := _helper.rect_for_units([0], 15.0)
	_selection.box_select(rect0, false)
	_runner.assert_equal(_selection.get_selected_count(), 1, "先选 1 个")
	# 追加选 unit 5
	var rect5 := _helper.rect_for_units([5], 15.0)
	_selection.box_select(rect5, true)  # additive
	_runner.assert_equal(_selection.get_selected_count(), 2, "追加后应为 2 个")
	_runner.assert_true(_selection.is_selected(_helper.units[0]), "unit 0 应被选中")
	_runner.assert_true(_selection.is_selected(_helper.units[5]), "unit 5 应被选中")
...
func _test_click_select() -> void:
	...
	_runner.assert_true(_selection.is_selected(_helper.units[2]), "应选中 unit 2")
	# 点击空地
...
func _test_clear_selection() -> void:
	if _selection == null:
		...
		return
	# 先选一些
	_selection.select_units([_helper.units[0], _helper.units[1]], false)
...
func _test_signal_emitted() -> void:
	...
	# 选中 unit 0
	_selection.select_unit(_helper.units[0], false)
	# 信号是同步发射的，应已收到
	_runner.assert_true(_signal_received, "应收到 selection_changed 信号")
	_runner.assert_equal(_signal_ids.size(), 1, "信号参数应有 1 个 id")
	_runner.assert_equal(_signal_ids[0], _helper.units[0].get_instance_id(), "id 应匹配 unit 0")
...
func _test_faction_filter() -> void:
	...
	for i in [0, 1, 2]:
		if _helper.units[i].has_method("set_faction"):
			_helper.units[i].set_faction(1)
	for i in [3, 4, 5]:
		if _helper.units[i].has_method("set_faction"):
			_helper.units[i].set_faction(2)
	# 限制只选阵营 1
	_selection.set_selectable_faction(1)
	# 框选所有单位
	var rect := _helper.rect_for_units([0, 1, 2, 3, 4, 5], 30.0)
	var selected: Array = _selection.box_select(rect, false)
	_runner.assert_equal(selected.size(), 3, "阵营过滤后应只选 3 个（阵营1）")
	for i in [0, 1, 2]:
		_runner.assert_true(_selection.is_selected(_helper.units[i]), "unit %d (阵营1) 应被选中" % i)
	for i in [3, 4, 5]:
		_runner.assert_true(not _selection.is_selected(_helper.units[i]), "unit %d (阵营2) 不应被选中" % i)
	# 恢复
...
func _test_dead_unit_removed() -> void:
	...
	_runner.assert_equal(_selection.get_selected_count(), 1, "先选 1 个")
	# 杀死 unit 0
	var health: Node = _helper.units[0].get_health() if _helper.units[0].has_method("get_health") else null
	if health == null:
		_runner.assert_true(false, "unit 0 无 HealthComponent")
		return
	health.take_damage(99999.0)  # 一击必杀
	_runner.assert_true(_helper.units[0].is_dead(), "unit 0 应已死亡")
