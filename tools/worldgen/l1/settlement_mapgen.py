...
开发期一次性工具（Python），产出：
  ...
  （seed 随机骨架：城门位/地标落位/街区分块/民居填充随机，骨架算法见 city_layout.py；
   地面分带/装饰物件由 city_decor.py 规划，烘焙为场景静态节点）。
...
def _band_style_params(style: int):
    """style → (base_color, alt_color, edge_soft)。配色 = city_decor.PALETTE 统一色板。"""
    pal = city_decor.PALETTE
    if style == 1:
        return pal["road"], pal["road"], 26.0
    if style == 2:
        return pal["stone"], pal["stone_alt"], 20.0
    if style == 3:
        return pal["farmland_soil"], pal["farmland_crop"], 18.0
    return pal["turf_base"], pal["turf_alt"], 30.0  # 草皮带边缘 30px 羽化进墙带硬地
...
def render_band_sections(decor: dict) -> tuple:
    ...
    for i, band in enumerate(decor["bands"]):
        style = band["style"]
        base, alt, edge = _band_style_params(style)
        sid = "BandMat_%d" % i
        r = band["rect"]
        subs += [
            ...
            "shader_parameter/seam_color = %s" % _color(pal["seam"]),
...
def render_decor_nodes(decor: dict) -> str:
    ...
    for i, lamp in enumerate(decor["lamps"]):
        lines += _group(i, "Lamp", lamp["x"], lamp["y"], [
            ("Glow", pal["lamp_glow"], _circle_pts(0, -46, 14), 0.10),
            ("Pole", pal["lamp_pole"], [(-1.6, 0), (1.6, 0), (1.6, -44), (-1.6, -44)]),
            ("Glass", pal["lamp_glass"], _circle_pts(0, -47, 5.5, 10)),
        ])
    for i, tree in enumerate(decor["trees"]):
        r = tree["r"]
        lines += _group(i, "Tree", tree["x"], tree["y"], [
            ("Trunk", pal["trunk"], [(-2, 0), (2, 0), (1.6, -r * 0.8), (-1.6, -r * 0.8)]),
            ("LeafBack", pal["leaf_a"], _circle_pts(-r * 0.45, -r * 1.05, r * 0.72)),
            ("LeafFront", pal["leaf_b"], _circle_pts(r * 0.38, -r * 0.95, r * 0.66)),
            ("LeafTop", pal["leaf_c"], _circle_pts(0, -r * 1.35, r * 0.78)),
        ])
    for i, bush in enumerate(decor["bushes"]):
        r = bush["r"]
        lines += _group(i, "Bush", bush["x"], bush["y"], [
            ("Body", pal["bush"], _circle_pts(0, -r * 0.5, r, 10)),
            ("Highlight", pal["bush_hi"], _circle_pts(-r * 0.25, -r * 0.75, r * 0.55, 10)),
        ])
    for i, c in enumerate(decor["clutter"]):
        x, y, kind = c["x"], c["y"], c["kind"]
        if kind == "barrel":
            shapes = [
                ("Body", pal["barrel"], [(-7, 0), (7, 0), (7, -16), (-7, -16)]),
                ("HoopTop", pal["barrel_hoop"], [(-7, -6), (7, -6), (7, -4.4), (-7, -4.4)]),
                ("HoopBot", pal["barrel_hoop"], [(-7, -12), (7, -12), (7, -10.4), (-7, -10.4)]),
            ]
        elif kind == "crate":
            shapes = [
                ("Face", pal["crate"], [(-8, 0), (8, 0), (8, -14), (-8, -14)]),
                ("Inner", pal["crate_in"], [(-5, -2.5), (5, -2.5), (5, -11), (-5, -11)]),
            ]
        else:  # hay
            shapes = [
                ("Mound", pal["hay"], _circle_pts(0, -5, 9, 10)),
                ("Top", pal["hay_hi"], _circle_pts(3.5, -6, 5.5, 10)),
            ]
