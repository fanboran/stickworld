...
YAW = 0.0              # 2D 侧视+微俯视游戏：水平不偏航（建筑正面朝观众）
TILT = 20.0            # 微俯视俯角（露屋面；横版卷轴里建筑正面朝观众，仅从上往下看）
...
def layout():
    ...
    for (name, wc) in B.PROBE_LIST:
        ...
        B.stickman(sb, x=stick_x, y=mx["y"][0] - 26.0)   # 站在建筑前缘（不占横向净距）
        sob = sb.to_object()
...
def main():
    ...
    print("\n=== 实测规格（单位 px；剪影为 yaw=0°/tilt=20° 投影）===")
