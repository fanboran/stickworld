...
flowchart TD
    ...
    GR --> WM["world_map（战略图）<br/>34 函数已写 / 未挂载运行时 ⚠️"]
...
| **Building** | `building_gen/`（BuildingDef 配方→节点树）+ `construction/`（生命周期） | 🟡 基础完整 | B0-B2 可用（仓库/城墙/兵营）；B3 模块系统/B4 多材质未做；**室内（0.9）无内容**（interior_mode 全 0、无室内摆设、无测试） |
...
| **world_map（战略图）** | 34 函数 api + 数据模型（ContinentData/RegionData/MapTileData）已写好 | **未接入运行时**（GameRoot 不装配）；现用 `world_map_panel.gd` 占位（按钮列表式目的地选择）；阶段 1 第一优先级 |
...
| **受击反馈粗粒度** | 击退+红闪+顿帧；无受击后仰姿态/无命中粒子/无伤害数字 | 视觉层迭代 |
| **装备/背包未做** | 临时配剑（所有火柴人同一把占位剑）；`weapons.tres`/`armors.tres` 未接入 | 见 §七 背包系统设计 |
...
| 2 | **world_map 未挂载** | 战略图（Tab/大世界）是占位按钮面板 | 阶段 1 首项：api.initialize + close_strategic_map 补齐 |
...
| 5 | **室内（0.9）断裂**：building def interior_mode=0、无室内内容、mega_interior 孤岛、零测试 | 原型循环"室内"环节不可验收 | 室内最小闭环（见待办事项.md） |
...
> 全量运行：`powershell -File tests/run_all.ps1`（17 套件，2026-08 全绿）
...
**未被任何测试覆盖**：technology、world_map、resources 供需、室内（0.9）、数值数据驱动、血条视觉。
