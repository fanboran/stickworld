...
### v3 期试错登记（防重走）：CC0 实拍贴图覆盖层（2026-09-17，创始人否决，已全量回退）

试点内容：ambientCG CC0 实拍 PBR 贴图接管 5 个痛点 key（thatch / plank_wall / stone / brick / iron）的 color/rough/normal 三路，程序化收尾框架（Tint/倒角磨白/逐体色变）原样复用；`agent/pbr-photo-pilot` 分支做完样片级 + 建筑级 A/B 验收后**未合并，分支/worktree/登记全部删除**。
裁决与教训：创始人验收结论「基本全不如之前的」——实拍贴图在物理微观上更真，但整体偏暗一档，茅草暗灰发糊远不如程序化金黄可读，25% 出厂门禁尺寸下程序化全面占优。**现行程序化材质库的「明亮干净 + 图形可读」风格基准优先于照片级微观真实感；后续会话不要再提"用实拍贴图替换程序化材质"类方案，除非创始人主动重开。**
