...
- **当前阶段**：**A 系列 + 观测接线批 + WorldBox WB1/WB2/WB6 三批全部收官合入 main；TeamAi 战场默认开已生效**——进入创始人观感验收与数值校准阶段（真实对局看阵营 AI 姿态/任务槽运作，F3 开 "TeamAi姿态HUD"，悬停单位看个体 AI 与目标评分字段）。WB3~WB5/WB7~WB10 已按任务线分流（见设计文档 §2.4 批次归属表），随各线立项；界面 W2~W4 待创始人裁决排序。
