extends Control
## 孤立调试场景（dev 层，手动跑）：HUD 部件两两不重叠。
## 原挂 tests/integration/ 但非 TestRunner 规范套件（自打印 FAIL、退出码不可靠、断言口径
## 停留在 zone 制之前的老布局），从未登记进 run_all 清单——已下沉 tests/dev/ 与自动化层
## 解耦，作为 UI 元素变更时的手动检查工具保留（不进任何 CI 清单）。
...
##
## 运行（带显示，需真实布局）：
##   godot --path stick-world res://tests/dev/test_ui_overlap.tscn --resolution 1920x1080
...
const _DriverScript: GDScript = preload("res://tests/dev/test_ui_overlap_driver.gd")
